...
from ..core.execution_engine import ExecutionContext, ExecutionEngine
from ..core.saidata_loader import SaidataLoader, SaidataNotFoundError
from ..core.saidata_repository_manager import SaidataRepositoryManager
...
def setup_logging(config, verbose: bool = False):
    ...
    if verbose:
        ...
        for logger_name in [
            ...
            "sai.core",
            "sai.utils",
            "sai.providers.loader",
            "sai.core.execution_engine",
            "sai.utils.execution_tracker",
            "sai.providers.base",
...
def status(ctx: click.Context, software: str, timeout: Optional[int], no_cache: bool):
    ...
    _execute_software_action(
        ctx, "status", software, timeout, requires_confirmation=False, use_cache=not no_cache
    )
...
def info(ctx: click.Context, software: str, timeout: Optional[int], no_cache: bool):
    ...
    """
    _execute_software_action(
        ctx, "info", software, timeout, requires_confirmation=False, use_cache=not no_cache
    )
...
def search(ctx: click.Context, term: str, timeout: Optional[int], no_cache: bool):
    ...
    _execute_software_action(
        ctx, "search", term, timeout, requires_confirmation=False, use_cache=not no_cache
    )
...
def list(ctx: click.Context, timeout: Optional[int], no_cache: bool):
    ...
    _execute_software_action(
        ctx, "list", "", timeout, requires_confirmation=False, use_cache=not no_cache
    )
...
def logs(ctx: click.Context, software: str, timeout: Optional[int], no_cache: bool):
    ...
    _execute_software_action(
        ctx, "logs", software, timeout, requires_confirmation=False, use_cache=not no_cache
    )
...
def version(ctx: click.Context, software: str, timeout: Optional[int], no_cache: bool):
    ...
    _execute_software_action(
        ctx, "version", software, timeout, requires_confirmation=False, use_cache=not no_cache
    )
...
@click.option(
    "--continue-on-error", is_flag=True, help="Continue executing remaining actions if one fails"
)
...
def apply(
    ...
    try:
        ...
        from ..core.action_loader import ActionFileError, ActionLoader
        from ..providers.loader import ProviderLoader
        ...
        if ctx.obj["output_json"]:
            ...
            formatter.print_execution_summary(
                result.total_actions, result.successful_actions, result.execution_time
            )
            ...
            if ctx.obj["verbose"]:
                for action_result in result.results:
                    ...
                    status_msg = click.style(
                        f"{status_symbol} {action_result.action_type} {action_result.software}",
                        fg=status_color,
                    )
                    ...
                    if not action_result.success and action_result.error:
                        error_msg = click.style(
                            f"    Error: {action_result.error}", fg="red", dim=True
                        )
        ...
        if ctx.obj["output_json"]:
            ...
            error_output = {"success": False, "error": str(e), "error_type": e.__class__.__name__}
...
def _get_provider_package_info(provider, saidata):
    ...
    try:
        ...
        if (
            hasattr(saidata, "providers")
            and saidata.providers
            and provider.name in saidata.providers
        ):
...
def _execute_software_action(
    ...
    try:
        ...
        if software:
            try:
                ...
                if not ctx.obj["quiet"] and ctx.obj["verbose"]:
                    click.echo(
                        f"Warning: No saidata found for '{software}', using basic execution",
                        err=True,
                    )
                # Create minimal saidata
                from ..models.saidata import Metadata, SaiData

                saidata = SaiData(version="0.2", metadata=Metadata(name=software))
        else:
            # Create minimal saidata for commands that don't require software name
            from ..models.saidata import Metadata, SaiData

            saidata = SaiData(version="0.2", metadata=Metadata(name=""))
        ...
        if (
            action in informational_actions
            and not requires_confirmation
            and not ctx.obj.get("provider")
        ):
        ...
        if (
            ...
            if not suitable_providers:
                ...
                if action_providers:
                    click.echo(
                        f"No providers can handle '{software}' for action '{action}'", err=True
                    )
                    click.echo(
                        f"Available providers for '{action}': {
                            ', '.join(
                                p.name for p in action_providers)}",
                        err=True,
                    )
                    click.echo(
                        f"Hint: Check if '{software}' has packages available for these providers",
                        err=True,
                    )
                else:
            ...
            if len(suitable_providers) == 1 or ctx.obj.get("provider"):
                ...
                if not selected:
                    ...
                    requested_provider = next(
                        (p for p in provider_instances if p.name == ctx.obj["provider"]), None
                    )
                    if requested_provider:
                        if requested_provider.has_action(action):
                            click.echo(
                                f"Provider '{
                                    ctx.obj['provider']}' cannot handle '{software}' for action '{action}'",
                                err=True,
                            )
                            click.echo(
                                f"Hint: Check if '{software}' has packages available for '{
                                    ctx.obj['provider']}'", err=True, )
                        else:
                            click.echo(
                                f"Provider '{
                                    ctx.obj['provider']}' does not support action '{action}'",
                                err=True,
                            )
                    else:
                        click.echo(
                            f"Requested provider '{ctx.obj['provider']}' not available", err=True
                        )
                    ctx.exit(1)
                ...
                try:
                    ...
                    if "command" in resolved:
                        command_msg = format_command_execution(
                            selected.name, resolved["command"], ctx.obj["verbose"]
                        )
                        ...
                        if "command" in first_step:
                            command_msg = format_command_execution(
                                selected.name, first_step["command"], ctx.obj["verbose"]
                            )
                            click.echo(f"Will execute: {command_msg}")
                        else:
                ...
                for provider in suitable_providers:
                    package_name, version_info, is_available = _get_provider_package_info(
                        provider, saidata
                    )
                ...
                for i, (provider, package_name, version_info) in enumerate(available_providers, 1):
                    ...
                    # Build display line
                    display_line = (
                        f"  {i}. {provider.name} (priority: {priority}) - package: {package_name}"
                    )
                    if version_info:
        ...
        if ctx.obj["output_json"]:
            ...
            if result.success:
                ...
                if (
                    ctx.obj["verbose"]
                    and result.commands_executed
                    and len(result.commands_executed) > 1
                ):
...
def _execute_informational_action_on_all_providers(
    ctx: click.Context,
    action: str,
    software: str,
    timeout: Optional[int],
    ...
    supporting_providers = [p for p in provider_instances if p.has_action(action)]
    ...
    if ctx.obj["output_json"]:
        ...
        output = {"action": action, "software": software, "providers": []}
            ...
            if result and result.success:
                ...
                if multiple_providers:
                    ...
                    formatter.print_error_message(
                        f"[{provider_name}] {result.message}", result.error_details
                    )
        ...
        if ctx.obj["output_json"]:
            ...
            if info["actions"]:
                ...
                for action_name, action_info in sorted(info["actions"].items()):
                    ...
                    timeout_info = (
                        f" (timeout: {action_info['timeout']}s)"
                        if action_info["timeout"] != 300
                        else ""
                    )
...
def providers_clear_cache(ctx: click.Context, provider: Optional[str]):
    ...
    try:
        ...
        if ctx.obj["output_json"]:
            ...
            error_output = {"success": False, "error": str(e), "provider": provider}
            click.echo(json.dumps(error_output, indent=2))
        else:
        ...
        if ctx.obj["output_json"]:
            ...
            if cache_info["cached_providers"]:
                ...
                for provider_info in cache_info["cached_providers"]:
                    age_str = (
                        f"({provider_info['age_hours']:.1f}h ago)"
                        if provider_info["age_hours"] < 24
                        else f"({provider_info['age_days']:.1f}d ago)"
                    )
...
def providers_refresh_cache(ctx: click.Context, provider: Optional[str]):
    ...
    try:
        ...
        for name, provider_data in providers.items():
            ...
            refreshed_count += 1
            results.append({"name": name, "available": is_available})
        ...
        if ctx.obj["output_json"]:
            ...
            output = {"success": True, "refreshed_count": refreshed_count, "providers": results}
            click.echo(json.dumps(output, indent=2))
        else:
            if not ctx.obj["quiet"]:
                if provider:
...
@click.option(
    "--config-file", type=click.Path(path_type=Path), help="Specific config file to modify"
)
@click.pass_context
...
@click.pass_context
def config_reset(
    ctx: click.Context, key: Optional[str], reset_all: bool, config_file: Optional[Path]
):
    ...
    try:
        ...
        if reset_all:
            ...
            if ctx.obj["output_json"]:
                ...
                output = {"success": True, "reset": "all", "config_file": str(save_path)}
                click.echo(json.dumps(output, indent=2))
            else:
        ...
        if ctx.obj["output_json"]:
            ...
            if key:
                if key in config_dict:
                    ...
                    click.echo(
                        json.dumps({"error": f'Configuration key "{key}" not found'}, indent=2)
                    )
                    ctx.exit(1)
            else:
                click.echo(json.dumps(config_dict, indent=2))
...
@cli.command()
@click.argument(
    "file", type=click.Path(exists=True, path_type=Path), shell_complete=complete_saidata_files
)
...
@click.pass_context
def cache_clear(
    ctx: click.Context, provider: Optional[str], saidata: Optional[str], clear_all: bool
):
    """Clear cache data."""
    try:
        from ..utils.cache import CacheManager
        ...
        if clear_all:
            ...
            if ctx.obj["output_json"]:
                ...
                if total_cleared > 0:
                    ...
                    formatter = create_output_formatter(ctx)
                    formatter.print_success_message(
                        f"Cleared {results['provider_cache_cleared']} provider cache entries"
                    )
                    formatter.print_success_message(
                        f"Cleared {results['saidata_cache_cleared']} saidata cache entries"
                    )
    ...
    try:
        ...
        if ctx.obj["output_json"]:
            ...
            if total_cleaned > 0:
                ...
                formatter = create_output_formatter(ctx)
                formatter.print_success_message(
                    f"Cleaned up {results['provider_cache_cleaned']} expired provider cache entries"
                )
                formatter.print_success_message(
                    f"Cleaned up {results['saidata_cache_cleaned']} expired saidata cache entries"
                )
        ...
        if ctx.obj["output_json"]:
            ...
            click.echo(
                f"{
                    click.style(
                        status_symbol,
                        fg=status_color)} Repository Status: {
                    click.style(
            ...
            if status.last_updated:
                ...
                click.echo(
                    f"  Last Updated: {status.last_updated.strftime('%Y-%m-%d %H:%M:%S')}{age_str}"
                )
            ...
            click.echo(
                f"  Cache Valid: {
                    click.style(
                        'Yes' if status.cache_valid else 'No',
                        fg='green' if status.cache_valid else 'yellow')}")
            ...
            if detailed:
                ...
                click.echo(
                    f"  Total Cached Repositories: {cache_status.get('total_repositories', 0)}"
                )
                ...
                click.echo(
                    f"  Network Connectivity: {
                        click.style(
                            'OK' if network_ok else 'Failed',
                            fg='green' if network_ok else 'red')}")
                ...
                if cached_repos:
                    ...
                    for repo in cached_repos:
                        ...
                        click.echo(
                            f"  • {repo['name']} - {click.style(repo_status, fg=repo_color)}"
                        )
...
@click.option(
    "--auto-update/--no-auto-update", default=None, help="Enable/disable automatic updates"
)
    ...
    try:
        ...
        if show:
            ...
            if ctx.obj["output_json"]:
                ...
                click.echo(
                    f"  Update Interval: {
                        config.saidata_update_interval}s ({
                        config.saidata_update_interval //
                        3600}h)")
        ...
        if update_interval:
            ...
            if not ctx.obj["quiet"]:
                click.echo(
                    f"Update interval set to: {update_interval}s ({
                        update_interval // 3600}h)")

        if not changes_made:
            click.echo(
                "No configuration changes specified. Use --show to view current settings.", err=True
            )
            ctx.exit(1)
...
@click.option(
    "--keep-days", type=int, default=7, help="Keep repositories newer than this many days"
)
@click.option(
    "--dry-run", is_flag=True, help="Show what would be cleaned without actually doing it"
)
@click.pass_context
    ...
    try:
        ...
        if dry_run:
            ...
            old_repos = [
                ...
                for repo in cached_repos
                if repo.get("last_updated")
                and datetime.fromisoformat(repo["last_updated"].replace("Z", "+00:00"))
                < cutoff_date
            ]
            ...
            if ctx.obj["output_json"]:
                ...
                click.echo(
                    f"Dry run: Would clean {
                        len(old_repos)} repositories older than {keep_days} days")
                for repo in old_repos:
                    click.echo(
                        f"  • {repo['name']} (last updated: {repo.get('last_updated', 'unknown')})"
                    )
        else:
            # Actually clean up
            ...
            if ctx.obj["output_json"]:
                ...
                result = {"cleaned_repositories": cleaned_count, "keep_days": keep_days}
                click.echo(json.dumps(result, indent=2))
            else:
                if cleaned_count > 0:
                    from ..utils.output_formatter import create_output_formatter
...
@click.option(
    "--branch", default="main", help="Branch name for specific repository (default: main)"
)
@click.pass_context
    ...
    try:
        ...
        if all:
            ...
            if ctx.obj["output_json"]:
                ...
                result = {"success": success, "repository_url": url, "branch": branch}
                click.echo(json.dumps(result, indent=2))
            else:
                if success:
                    from ..utils.output_formatter import create_output_formatter
                    ...
                    formatter = create_output_formatter(ctx)
                    formatter.print_success_message(
                        f"Cleared cache for repository: {url} (branch: {branch})"
                    )
                else:
