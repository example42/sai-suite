...
class ProviderLoadError(Exception):
    ...
    def __init__(
        self, provider_file: Path, message: str, original_error: Optional[Exception] = None
    ):
        self.provider_file = provider_file
        self.message = message
...
class ProviderLoader:
    ...
    def __init__(self, schema_path: Optional[Path] = None, enable_caching: bool = True):
        ...
        self.schema_path = (
            schema_path
            or Path(__file__).parent.parent.parent / "schemas" / "providerdata-0.1-schema.json"
        )
    ...
    def _load_schema(self) -> None:
        ...
        try:
            if not self.schema_path.exists():
                logger.warning(
                    f"Provider schema not found at {self.schema_path}, validation will be skipped"
                )
                return
    ...
    def validate_yaml_structure(self, data: dict, provider_file: Path) -> None:
        ...
        try:
            ...
            raise ProviderValidationError(
                provider_file, "JSON schema validation failed", details
            ) from e
    ...
    def validate_pydantic_model(self, data: dict, provider_file: Path) -> ProviderData:
        ...
        try:
            ...
            raise ProviderValidationError(
                provider_file, "Pydantic model validation failed", details
            ) from e
        ...
        try:
            ...
            if file_size > max_size:
                raise ProviderLoadError(
                    provider_file, f"Provider file too large: {file_size} bytes (max: {max_size})"
                )
            ...
            if not isinstance(data, dict):
                raise ProviderLoadError(
                    provider_file, f"Expected YAML object, got {type(data).__name__}"
                )
            ...
            logger.info(
                f"Successfully loaded provider: {provider_data.provider.name} from {provider_file}"
            )
            return provider_data

        except (yaml.YAMLError, UnicodeDecodeError) as e:
            raise ProviderLoadError(provider_file, f"Failed to parse YAML: {e}", e) from e
        except FileNotFoundError as e:
            raise ProviderLoadError(provider_file, "File not found", e) from e
        except PermissionError as e:
            raise ProviderLoadError(provider_file, "Permission denied", e) from e
    ...
    def load_all_providers(
        self, additional_directories: Optional[List[Path]] = None
            ...
            try:
                ...
                for name, provider_data in providers.items():
                    if name in all_providers:
                        logger.info(
                            f"Provider '{name}' from {directory} overrides " f"previous definition"
                        )
        ...
        logger.info(
            f"Loaded {len(all_providers)} total providers from {len(directories)} directories"
        )
