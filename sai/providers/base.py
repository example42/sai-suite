...
from ..models.provider_data import Action, ProviderData
...
class BaseProvider:
    ...
    def _check_package_availability(self, saidata: SaiData) -> bool:
        ...
        if hasattr(saidata, "packages") and saidata.packages:
            logger.debug(
                f"Provider '{self.name}' can use general packages for '{saidata.metadata.name}'"
            )
            return True
        ...
        if hasattr(saidata, "metadata") and saidata.metadata and saidata.metadata.name:
            logger.debug(
                f"Provider '{self.name}' can use metadata name for '{saidata.metadata.name}'"
            )
            return True
    ...
    def get_priority(self) -> int:
        ...
        if (
            hasattr(self.provider_data.provider, "priority")
            and self.provider_data.provider.priority is not None
        ):
        ...
        if use_cache:
            try:
                ...
                if cached_info is not None:
                    logger.debug(
                        f"Using cached availability for provider '{
                            self.name}': {
                            cached_info['available']}")
    ...
    def _get_main_executable(self) -> Optional[str]:
        ...
        if (
            hasattr(self.provider_data.provider, "executable")
            and self.provider_data.provider.executable
        ):
        ...
        logger.debug(
            f"No explicit executable found for provider '{self.name}', using provider name"
        )
    ...
    def _test_functionality(self) -> bool:
        ...
        for action_name in priority_actions:
            if action_name in self.provider_data.actions:
                ...
                if action.validation and action.validation.command:
                    ...
                    if (
                        "{{" not in action.validation.command
                        and "{%" not in action.validation.command
                    ):
                        ...
                        logger.debug(
                            f"Testing provider '{
                                self.name}' with validation command: {test_command}")
        ...
        for action_name, action in self.provider_data.actions.items():
            if action.validation and action.validation.command:
                ...
                if "{{" not in action.validation.command and "{%" not in action.validation.command:
                    ...
                    logger.debug(
                        f"Testing provider '{self.name}' with validation command: {test_command}"
                    )
        ...
        if main_executable:
            ...
            for cmd in version_commands:
                try:
                    ...
                    result = check_executable_functionality(
                        main_executable, cmd, expected_exit_code=0, timeout=10
                    )
                    if result:
                        return True
                except Exception:
        ...
        logger.debug(
            f"No suitable validation command found for provider '{self.name}', assuming functional"
        )
        return True
    ...
    def resolve_action_templates(
        self, action_name: str, saidata: SaiData, additional_context: Optional[Dict] = None
    ) -> Dict[str, str]:
        ...
        try:
            ...
            logger.error(
                f"Failed to resolve templates for action '{action_name}' in provider '{
                    self.name}': {e}")
            raise

    def resolve_template(
        self, template_str: str, saidata: SaiData, additional_context: Optional[Dict] = None
    ) -> str:
...
class ProviderFactory:
    ...
    def create_available_providers(self, additional_directories: List = None) -> List[BaseProvider]:
        ...
        logger.info(
            f"Found {
                len(available_providers)} available providers out of {
                len(all_providers)} total")
        return available_providers

    def detect_providers(
        self, additional_directories: List = None, use_cache: bool = True
        ...
        logger.info(
            f"Detection complete: {available_count} available out of {
                len(provider_info)} total providers")
