...
class Actions(BaseModel):
    ...
    def validate_action_structure(cls, values):
        ...
        for action_name, action_list in values.items():
            ...
            if not action_name_pattern.match(action_name):
                raise ValueError(f"Invalid action name '{action_name}'. Must start with a letter and contain only letters, numbers, underscores, and hyphens.")
    ...
    def get_action_types(self) -> List[str]:
        ...
        if hasattr(self, '__pydantic_extra__'):
            return [
                action_type for action_type, action_list in self.__pydantic_extra__.items()
