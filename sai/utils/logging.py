...
class SaiLogger:
    ...
    def log_execution_start(
        self, action: str, software: str, provider: str, context: Optional[Dict[str, Any]] = None
    ) -> None:
    ...
    def log_execution_end(
        ...
        message = (
            f"Execution {status}: {action} {software} using {provider} ({execution_time:.2f}s)"
        )
    ...
    def log_command_execution(
        ...
        success = exit_code == 0
        message = f"Command {
            'succeeded' if success else 'failed'}: {command} (exit: {exit_code}, time: {
            execution_time:.2f}s)"
...
def configure_logger(
    name: str, level: int, add_console_handler: bool = True, log_file: Optional[Path] = None
) -> logging.Logger:
