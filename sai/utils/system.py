...
def check_executable_functionality(
    executable: str, test_command: List[str], expected_exit_code: int = 0, timeout: int = 30
) -> bool:
    ...
    try:
        ...
        if not all(isinstance(arg, str) for arg in test_command):
            logger.warning(
                f"Invalid command arguments for '{executable}': non-string arguments detected"
            )
            return False
...
def detect_offline_mode() -> Tuple[bool, str]:
    ...
    test_hosts = [("github.com", 443), ("gitlab.com", 443), ("bitbucket.org", 443)]
    ...
    logger.debug(
        f"Network connectivity OK: {accessible_count}/{len(test_hosts)} services accessible"
    )
...
class NetworkConnectivityTracker:
    ...
    def record_failure(self) -> None:
        ...
        logger.debug(
            f"Network failure recorded (count: {self._failure_count}, "
    ...
    def should_retry(self) -> Tuple[bool, float]:
        ...
        if (
            ...
            and time.time() - self._last_failure_time > self._reset_threshold
        ):
