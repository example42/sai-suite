...
class RepositorySecurityValidator:
        ...
        if not is_known_safe:
            if self.security_level == SecurityLevel.STRICT:
                ...
                recommendations.append(
                    "Consider using repositories from trusted sources like GitHub, GitLab, or Bitbucket"
                )
            else:
    ...
    def validate_file_path(
        self, file_path: str, base_path: Optional[Path] = None
...
class GitSignatureVerifier:
    ...
    def verify_commit_signature(
        self, repo_path: Path, commit_hash: Optional[str] = None
    ) -> GitSignatureInfo:
        ...
        if not self.is_git_available():
            return GitSignatureInfo(
                is_signed=False, is_valid=False, error_message="Git is not available"
            )
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "no signature found" in result.stderr.lower():
                    return GitSignatureInfo(
                        is_signed=False, is_valid=False, error_message="Commit is not signed"
                    )
                else:
                    return GitSignatureInfo(
                        is_signed=True, is_valid=False, error_message=result.stderr.strip()
                    )

        except subprocess.TimeoutExpired:
            return GitSignatureInfo(
                is_signed=False, is_valid=False, error_message="Signature verification timed out"
            )
        except Exception as e:
            return GitSignatureInfo(
                is_signed=False, is_valid=False, error_message=f"Signature verification failed: {e}"
            )
    ...
    def verify_tag_signature(self, repo_path: Path, tag_name: str) -> GitSignatureInfo:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if "no signature found" in result.stderr.lower():
                    return GitSignatureInfo(
                        is_signed=False, is_valid=False, error_message="Tag is not signed"
                    )
                else:
                    return GitSignatureInfo(
                        is_signed=True, is_valid=False, error_message=result.stderr.strip()
    ...
    def validate_file_checksum(
        ...
        try:
            ...
            if is_valid:
                ...
                logger.warning(
                    f"Checksum validation failed: expected {expected_checksum}, got {calculated_checksum}"
                )
