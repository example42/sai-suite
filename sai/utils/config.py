...
class ConfigManager:
    ...
    def _load_env_overrides(self) -> None:
        ...
        env_overrides = {
            ...
            "SAI_REQUIRE_CONFIRMATION": lambda val: setattr(
                self._config, "require_confirmation", val.lower() == "true"
            ),
            # Repository configuration overrides
            "SAI_REPOSITORY_URL": lambda val: setattr(self._config, "saidata_repository_url", val),
            "SAI_REPOSITORY_BRANCH": lambda val: setattr(
                self._config, "saidata_repository_branch", val
            ),
            "SAI_REPOSITORY_AUTH_TYPE": lambda val: setattr(
                self._config, "saidata_repository_auth_type", val
            ),
            "SAI_AUTO_UPDATE": lambda val: setattr(
                self._config, "saidata_auto_update", val.lower() == "true"
            ),
            "SAI_UPDATE_INTERVAL": lambda val: setattr(
                self._config, "saidata_update_interval", int(val)
            ),
            "SAI_OFFLINE_MODE": lambda val: setattr(
                self._config, "saidata_offline_mode", val.lower() == "true"
            ),
            "SAI_REPOSITORY_CACHE_DIR": lambda val: setattr(
                self._config, "saidata_repository_cache_dir", Path(val)
            ),
            "SAI_SHALLOW_CLONE": lambda val: setattr(
                self._config, "saidata_shallow_clone", val.lower() == "true"
            ),
            "SAI_REPOSITORY_TIMEOUT": lambda val: setattr(
                self._config, "saidata_repository_timeout", int(val)
            ),
        }
    ...
    def validate_config(self) -> List[str]:
        ...
        try:
            ...
            issues.append(
                f"Cannot create repository cache directory: {config.saidata_repository_cache_dir}"
            )
        ...
        if config.saidata_repository_auth_type and not config.saidata_repository_auth_data:
            issues.append(
                "Repository authentication type specified but no authentication data provided"
            )
        ...
        if not saidata_paths_exist and not config.saidata_auto_update:
            issues.append(
                "No saidata paths exist and auto-update is disabled - you may need to initialize saidata or enable auto-update"
            )
