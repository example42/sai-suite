...
from ..utils.execution_tracker import ExecutionStatus, get_execution_tracker
...
class ExecutionEngine:
    ...
    def __init__(self, providers: List[BaseProvider], config: Optional[Any] = None):
        ...
        logger.info(
            f"ExecutionEngine initialized with {len(self.providers)} providers "
    ...
    def execute_action(self, context: ExecutionContext) -> ExecutionResult:
        ...
        try:
            ...
            execution_id = self.execution_tracker.start_execution(
                ...
                additional_context={
                    "saidata_version": getattr(context.saidata, "version", None)
                    if context.saidata
                    else None
                },
            )
            ...
            logger.info(
                f"Executing action '{context.action}' for '{context.software}' "
            ...
            logger.info(
                f"Action execution completed: {result.status} in {result.execution_time:.2f}s"
            )
            return result
    ...
    def _select_provider(self, context: ExecutionContext) -> BaseProvider:
                ...
                if provider.name == context.provider:
                    if provider.has_action(context.action):
                        ...
                        raise ProviderSelectionError(
                            f"Provider '{
                                context.provider}' does not support action '{
                                context.action}'")

            raise ProviderSelectionError(f"Requested provider '{context.provider}' not available")

        # Find providers that support the action
        suitable_providers = [p for p in self.available_providers if p.has_action(context.action)]
        ...
        logger.debug(
            f"Selected provider '{selected.name}' (priority: {selected.get_priority()}) "
    ...
    def _dry_run_action(
        ...
        try:
            ...
            commands = []
            message_parts = [
                f"DRY RUN: Would execute action '{context.action}' using provider '{provider.name}'"
            ]
        ...
        try:
            ...
            if action.steps:
                ...
                result = self._execute_steps(
                    resolved["steps"], action, commands_executed, context, execution_id
                )
            elif "command" in resolved:
                # Single command execution
                result = self._execute_command(
                    resolved["command"], action, commands_executed, context, execution_id
                )
            elif "script" in resolved:
                # Script execution
                result = self._execute_script(
                    resolved["script"], action, commands_executed, context, execution_id
                )
            else:
    ...
    def _run_secure_command(
        ...
        try:
            ...
            if not quiet:
                safe_cmd = " ".join(
                    arg if not self._is_sensitive_arg(arg) else "[REDACTED]" for arg in final_args
                )
    ...
    def _validate_command_security(
        self, cmd_args: List[str], requires_root: bool
    ) -> Dict[str, Any]:
        ...
        for arg in cmd_args:
            ...
            for pattern in dangerous_patterns:
                if pattern in arg:
                    return {
                        "valid": False,
                        "error": f'Potentially dangerous pattern "{pattern}" in argument',
                    }
    ...
    def _handle_privilege_escalation(self, cmd_args: List[str], requires_root: bool) -> List[str]:
        ...
        sudo_args = ["sudo", "-n", "--"]  # Non-interactive mode  # End of sudo options
    ...
    def _handle_process_execution(
        self, process: subprocess.Popen, timeout: int, verbose: bool
    ) -> Dict[str, Any]:
        ...
        if condition.startswith("command_success:"):
            ...
            try:
                result = subprocess.run(
                    shlex.split(command), capture_output=True, timeout=10, shell=False
                )
