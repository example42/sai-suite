...
class GitRepositoryHandler:
    ...
    def clone_repository(
        ...
        logger.debug(
            f"Clone parameters: branch={branch}, shallow={shallow}, auth_type={auth_type}, verify_signatures={verify_signatures}"
        )
    ...
    def update_repository(
        ...
        if repo_info:
            logger.debug(
                f"Repository info before update: branch={
                    repo_info.branch}, commit={
        ...
        self._execute_git_command(status_cmd)
        ...
        if reset_result.success:
            ...
            if updated_repo_info and repo_info:
                if updated_repo_info.commit_hash != repo_info.commit_hash:
                    logger.info(
                        f"Repository updated: {repo_info.commit_hash} -> {updated_repo_info.commit_hash}"
                    )
                else:
    ...
    def _setup_git_environment(
        self, auth_type: Optional[RepositoryAuthType], auth_data: Optional[Dict[str, str]]
        ...
        if stored_credentials:
            ...
            if auth_type_str == "ssh":
                return self._setup_git_environment(
                    RepositoryAuthType.SSH, {"ssh_key_path": stored_credentials.get("ssh_key_path")}
                )
            elif auth_type_str == "token":
                return self._setup_git_environment(
                    RepositoryAuthType.TOKEN, {"token": stored_credentials.get("token")}
                )
        ...
        if not self.signature_verifier.is_gpg_available():
            ...
            if head_signature.is_signed:
                if head_signature.is_valid:
                    ...
                    warnings.append(
                        f"HEAD commit signature is invalid: {head_signature.error_message}"
                    )
                    recommendations.append(
                        "Verify the repository source and consider using a different branch or commit"
                    )
            else:
                warnings.append("HEAD commit is not signed")
                recommendations.append(
                    "Consider using repositories with signed commits for better security"
                )
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    for tag in tags:
                        tag_signature = self.signature_verifier.verify_tag_signature(
                            repo_path, tag.strip()
                        )
                    ...
                    if signed_tags > 0:
                        logger.info(
                            f"Found {signed_tags} signed tags out of {len(tags)} recent tags"
                        )
                    else:
            ...
            if auth_type == RepositoryAuthType.SSH:
                ...
                if ssh_key_path:
                    return self.credential_manager.store_ssh_key(
                        repository_url, ssh_key_path, auth_data.get("username")
                    )
    ...
    def _execute_git_command(
        self, cmd: list, env: Optional[Dict[str, str]] = None, cwd: Optional[Path] = None
    ) -> GitOperationResult:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=self.timeout, env=full_env, cwd=cwd
            )
            ...
            logger.error(error_msg)
            return GitOperationResult(success=False, message=error_msg, return_code=-1)
    ...
    def _execute_git_command_with_retry(
        ...
        for attempt in range(self.max_retries):
            ...
            if attempt < self.max_retries - 1:
                ...
                delay = 2**attempt
                logger.warning(
                    f"Git command failed (attempt {attempt + 1}/{self.max_retries}), retrying in {delay}s..."
                )
                time.sleep(delay)

        logger.error(f"Git command failed after {self.max_retries} attempts")
        return last_result or GitOperationResult(
            success=False, message=f"Command failed after {self.max_retries} attempts"
        )

    def _verify_repository_integrity(
        self,
        repo_dir: Path,
        ...
        try:
            ...
            if not (repo_dir / ".git").exists():
                logger.error(
                    f"Repository integrity check failed: .git directory missing in {repo_dir}"
                )
                return False
            ...
            if not repo_info:
                logger.error(
                    f"Repository integrity check failed: unable to get repository info for {repo_dir}")
                return False
            ...
            if expected_url and repo_info.remote_url:
                ...
                if normalized_expected != normalized_actual:
                    logger.error(
                        f"Repository integrity check failed: URL mismatch. "
            ...
            if expected_branch and repo_info.branch != expected_branch:
                logger.error(
                    f"Repository integrity check failed: branch mismatch. "
            ...
            if not repo_info.commit_hash:
                logger.error(
                    f"Repository integrity check failed: no valid commit hash for {repo_dir}"
                )
                return False
    ...
    def _log_git_error_details(
        self, result: GitOperationResult, repository_url: str, operation: str
    ) -> None:
    ...
    def _analyze_git_error(self, stderr: str, repository_url: str, operation: str) -> None:
        ...
        if any(
            ...
            logger.info(
                "  - For SSH: verify your SSH key is configured and added to your SSH agent"
            )
        ...
        # Repository not found errors
            ...
            phrase in stderr_lower
            for phrase in ["repository not found", "does not exist", "not found"]
        ):
        ...
        # Branch/reference errors
            ...
            phrase in stderr_lower
            for phrase in ["remote branch", "does not exist", "reference not found"]
        ):
        ...
        # Disk space errors
            ...
            phrase in stderr_lower
            for phrase in ["no space left", "disk full", "insufficient space"]
        ):
        ...
        # Permission errors
            ...
            phrase in stderr_lower
            for phrase in ["permission denied", "access denied", "operation not permitted"]
        ):
