...
from ..utils.errors import (
    TarballDownloadError, RepositoryAuthenticationError, RepositoryNetworkError,
    ChecksumValidationError, RepositoryNotFoundError, RepositoryIntegrityError,
    SecurityError
)
from ..utils.security import (
    RepositorySecurityValidator, ChecksumValidator, PathTraversalProtector,
    SecurityLevel, SecurityValidationResult
)
...
class ProgressReporter:
    ...
    def __call__(self, block_num: int, block_size: int, total_size: int):
        ...
        if total_size > 0 and block_num % 100 == 0:
            ...
            logger.debug(f"Download progress: {percent:.1f}% ({self.downloaded}/{total_size} bytes)")
...
class TarballRepositoryHandler:
    ...
    def __init__(self, timeout: int = 300, max_retries: int = 3, security_level: SecurityLevel = SecurityLevel.MODERATE):
        ...
        logger.debug(f"Tarball repository handler initialized with security level: {security_level}")
    ...
    def get_latest_release_info(
        ...
        if not url_validation.is_valid:
            ...
            return TarballOperationResult(
                success=False,
                message=error_msg,
                error_details="Security validation failed"
            )
        ...
        try:
            ...
            if auth_type and auth_data:
                ...
                if stored_credentials:
                    ...
                    if auth_type_str == 'token':
                        self._add_auth_headers(request, RepositoryAuthType.TOKEN, {'token': stored_credentials.get('token')})
                        auth_added = True
                    elif auth_type_str == 'basic':
                        self._add_auth_headers(request, RepositoryAuthType.BASIC, {
            ...
            if not release_info:
                ...
                logger.error(error_msg)
                logger.debug(f"Available assets: {[asset.get('name', 'unnamed') for asset in data.get('assets', [])]}")
                return TarballOperationResult(
                    success=False,
                    message=error_msg,
            ...
            logger.debug(f"Release details: download_url={release_info.download_url}, size={release_info.size}")
    ...
    def download_and_extract_release(
        ...
        try:
            ...
            logger.info(f"Downloading release {release_info.tag_name} from {release_info.download_url}")
            ...
            if release_info.checksum:
                checksum_result = self._verify_checksum_enhanced(
                    temp_file,
                    release_info.checksum,
                    release_info.checksum_algorithm
                )
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    delay = 2 ** attempt
                    logger.warning(f"Download failed (attempt {attempt + 1}/{self.max_retries}), retrying in {delay}s: {e}")
    ...
    def _verify_checksum(
        ...
        try:
            ...
            if calculated_checksum == expected_checksum:
                ...
                return TarballOperationResult(
                    success=True,
                    message=f"Checksum verification passed ({algorithm})"
                )
            else:
                return TarballOperationResult(
                    success=False,
            ...
            return TarballOperationResult(
                success=False,
                message=f"Checksum verification error: {e}",
                error_details=str(e)
            )
    ...
    def _extract_archive_atomically(
        ...
        try:
            ...
            return TarballOperationResult(
                success=False,
                message=f"Archive extraction failed: {e}",
                error_details=str(e)
            )
    ...
    def _is_temporary_network_error(self, error_message: str) -> bool:
        ...
        temporary_indicators = [
            ...
            'temporary failure', 'network is unreachable', 'host is down',
            'connection aborted', 'broken pipe'
        ]
        
        # Permanent configuration issues
        permanent_indicators = [
            'name or service not known', 'no such host', 'invalid hostname',
        ...
        if total > 0:
            ...
            for point in progress_points:
                if abs(percent - point) < 1:  # Within 1% of progress point
                    logger.info(f"Download progress: {percent:.1f}% ({mb_downloaded:.1f}MB / {mb_total:.1f}MB)")
                    break
    
    def _validate_download_integrity(self, file_path: Path, expected_size: Optional[int] = None) -> bool:
        ...
        try:
            ...
            logger.debug(f"Download integrity validation passed for {file_path} ({actual_size} bytes)")
            return True
    ...
    def _extract_tar_secure(self, archive_path: Path, target_dir: Path, path_protector: PathTraversalProtector):
    ...
    def _extract_zip_secure(self, archive_path: Path, target_dir: Path, path_protector: PathTraversalProtector):
            ...
            for member_name in zip_file.namelist():
                ...
                try:
                    ...
                    if info.file_size > 100 * 1024 * 1024:  # 100MB limit
                        logger.warning(f"Large file in archive: {member_name} ({info.file_size} bytes)")
                    
                    # Check compression ratio for zip bomb detection
                    if info.compress_size > 0:
                        ratio = info.file_size / info.compress_size
                        if ratio > 100:  # Suspicious compression ratio
                            logger.warning(f"High compression ratio for {member_name}: {ratio:.1f}:1")
                            if ratio > 1000:  # Very suspicious
                                blocked_members.append(f"{member_name}: suspicious compression ratio ({ratio:.1f}:1)")
                                continue
                except Exception as e:
    ...
    def calculate_file_checksums(self, file_path: Path, algorithms: List[str] = None) -> Dict[str, str]:
