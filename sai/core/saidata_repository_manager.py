...
from .git_repository_handler import GitRepositoryHandler, GitOperationResult, RepositoryInfo
from .tarball_repository_handler import TarballRepositoryHandler, TarballOperationResult
from .saidata_loader import SaidataLoader, SaidataNotFoundError
from .repository_cache import RepositoryCache
from ..models.config import SaiConfig, RepositoryAuthType
from ..models.saidata import SaiData
from ..utils.errors import (
    RepositoryError, RepositoryNotFoundError, RepositoryAuthenticationError,
    RepositoryNetworkError, RepositoryIntegrityError, RepositoryStructureError,
    RepositoryCacheError
)
from ..utils.system import (
    check_network_connectivity, check_url_accessibility, detect_offline_mode,
...
class SaidataRepositoryManager:
    ...
    def get_saidata(self, software_name: str, force_update: bool = False) -> Optional[SaiData]:
        ...
        logger.debug(f"Repository status: {health_check.status}, cache_valid={health_check.cache_valid}")
        ...
        try:
            ...
            if (not force_update and self._should_retry_with_update() and 
                not self.is_offline_mode()):
                logger.info(f"Saidata not found for {software_name}, attempting repository update...")
                try:
                    update_result = self.update_repository(force=True)
                    if update_result:
                        logger.debug(f"Repository updated, retrying saidata load for {software_name}")
                ...
                logger.debug(f"Saidata not found for {software_name} and offline mode active - no retry attempted")
    ...
    def update_repository(self, force: bool = False) -> bool:
        ...
        logger.debug(f"Update parameters: force={force}, offline_mode={self.config.saidata_offline_mode}")
        ...
        try:
            ...
            if self.git_handler.is_git_available():
                ...
                if result:
                    ...
                    logger.info(f"Git repository update completed successfully in {update_duration:.2f}s")
            ...
            if result:
                update_duration = (datetime.now() - update_start_time).total_seconds()
                logger.info(f"Tarball repository update completed successfully in {update_duration:.2f}s")
    ...
    def configure_repository(
        ...
        try:
            # Validate URL format
            if not url or not any(url.startswith(prefix) for prefix in ['http://', 'https://', 'git://', 'ssh://', 'git@']):
    ...
    def cleanup_old_repositories(self, keep_days: int = 7) -> int:
        ...
        if total_cleaned > 0:
            logger.info(f"Cleaned up {total_cleaned} repository caches "
    ...
    def get_network_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            'last_network_check': self._last_network_check.isoformat() if self._last_network_check else None,
        }
    ...
    def is_offline_mode(self) -> bool:
        ...
        if (self._last_network_check is None or 
            (now - self._last_network_check).total_seconds() > self._network_check_interval):
    ...
    def warn_about_stale_cache(self, cache_age: timedelta) -> None:
        ...
        if age_days > 7:
            ...
            logger.warning(
                f"Using cached repository data that is {age_days} days, {age_hours % 24:.0f} hours old. "
                f"Data may be outdated."
            )
        elif age_hours > 6:
            logger.info(
                f"Using cached repository data that is {age_hours:.0f} hours old."
            )
        else:
    ...
    def _update_with_git(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"Git update parameters: auth_type={auth_type}, shallow={self.config.saidata_shallow_clone}")
            ...
            if result.success:
                ...
                self._network_tracker.record_success()
                self._log_repository_operation_summary(operation, True, duration, {
                    'method': 'git',
                ...
                self._network_tracker.record_failure()
                self._log_repository_operation_summary(operation, False, duration, {
                    'method': 'git',
            ...
            self._network_tracker.record_failure()
            self._log_repository_operation_summary("git_operation", False, duration, {
                'method': 'git',
                'exception': str(e)
            })
            return False
    ...
    def _update_with_tarball(self) -> bool:
        ...
        try:
            ...
            def progress_callback(downloaded: int, total: int):
                if total > 10 * 1024 * 1024:  # Only log for downloads > 10MB
                    ...
                    if percent % 25 < 1:  # Log every 25%
                        ...
                        logger.info(f"Download progress: {percent:.0f}% ({mb_downloaded:.1f}MB / {mb_total:.1f}MB)")
            ...
            if result.success:
                ...
                self._log_repository_operation_summary("tarball_download", True, duration, {
                    'method': 'tarball',
                    'release_tag': result.release_info.tag_name if result.release_info else None,
                    'repository_size_mb': self._get_repository_size_mb()
                })
                return True
            else:
                ...
                self._log_repository_operation_summary("tarball_download", False, duration, {
                    'method': 'tarball',
                    'error_message': result.message,
            ...
            self._network_tracker.record_failure()
            self._log_repository_operation_summary("tarball_operation", False, duration, {
                'method': 'tarball',
                'exception': str(e)
            })
            return False
    ...
    def _validate_repository_structure(self) -> bool:
        ...
        try:
            ...
            if not software_dir.exists():
                logger.warning(f"Expected 'software' directory not found in repository: {self.repository_path}")
            ...
            logger.info(f"Repository structure validation passed ({valid_files}/{len(sample_files)} sample files valid)")
            return True
            ...
            if software_dir.exists():
                ...
                if prefix_dir.exists():
                    ...
                    if software_specific_dir.exists():
                        logger.info(f"Software directory exists but no default.yaml found: {software_specific_dir}")
                        # List what files are in the directory
                        files = list(software_specific_dir.iterdir())
                        if files:
                            logger.debug(f"Files in {software_specific_dir}: {[f.name for f in files]}")
                    else:
                        logger.info(f"Software-specific directory does not exist: {software_specific_dir}")
                        ...
                        if available_software:
                            logger.debug(f"Available software with prefix '{prefix}': {available_software[:10]}")
                else:
        ...
        try:
            ...
            for prefix_dir in software_dir.iterdir():
                if prefix_dir.is_dir():
                    for software_specific_dir in prefix_dir.iterdir():
                        if software_specific_dir.is_dir() and (software_specific_dir / "default.yaml").exists():
            ...
            if not suggestions:
                for name in available_software:
                    ...
                    if (software_lower.startswith(name_lower[:3]) or 
                        name_lower.startswith(software_lower[:3]) or
                        software_lower.endswith(name_lower[-3:]) or
                        name_lower.endswith(software_lower[-3:])):
                        suggestions.append(name)
    ...
    def _log_repository_operation_summary(self, operation: str, success: bool, 
