...
class SaidataPath:
    ...
    def from_software_name(cls, software_name: str, base_path: Path) -> 'SaidataPath':
        ...
        if not normalized_name.replace('-', '').replace('_', '').replace('.', '').isalnum():
            raise ValueError(f"Invalid software name: {software_name}. Must contain only alphanumeric characters, hyphens, underscores, and dots.")
    ...
    def validate_path(self) -> List[str]:
        ...
        expected_prefix = self.software_name[:2] if len(self.software_name) >= 2 else self.software_name
        if path_parts[-3] != expected_prefix:
            errors.append(f"Prefix directory '{path_parts[-3]}' does not match expected '{expected_prefix}'")
        
        if path_parts[-2] != self.software_name:
            errors.append(f"Software directory '{path_parts[-2]}' does not match software name '{self.software_name}'")
    ...
    def __repr__(self) -> str:
        ...
        return f"SaidataPath(software_name='{self.software_name}', hierarchical_path='{self.hierarchical_path}')"
...
class HierarchicalPathResolver:
    ...
    def find_saidata_files(self, software_name: str) -> List[Path]:
        ...
        for search_path in self.search_paths:
            ...
            try:
                ...
                logger.debug(f"Error creating SaidataPath for '{software_name}' in {search_path}: {e}")
                continue
            except Exception as e:
                logger.warning(f"Unexpected error searching for '{software_name}' in {search_path}: {e}")
                continue
        
        logger.info(f"Found {len(found_files)} saidata files for '{software_name}'")
        return found_files
    
    def get_expected_path(self, software_name: str, base_path: Optional[Path] = None) -> SaidataPath:
    ...
    def validate_software_name(self, software_name: str) -> List[str]:
        ...
        if not normalized_name.replace('-', '').replace('_', '').replace('.', '').isalnum():
            errors.append("Software name must contain only alphanumeric characters, hyphens, underscores, and dots")
