...
from saigen.repositories.indexer import RAGIndexer, RAGContextBuilder
...
class TestRAGIndexer:
    ...
    async def test_search_similar_packages(self, temp_index_dir, sample_packages):
        ...
        with patch.object(indexer, '_get_model') as mock_get_model, \
             patch.object(indexer, '_load_package_index') as mock_load_index:
...
class TestRAGContextBuilder:
    ...
    @pytest.mark.asyncio
    async def test_build_context(self, context_builder, mock_indexer, sample_packages, sample_saidata):
