...
class TestGenerationEnginePerformance:
    ...
    async def test_single_generation_performance(self, mock_generation_engine):
        ...
        request = GenerationRequest(software_name="nginx", llm_provider=LLMProvider.OPENAI)
        ...
        )  # Should complete in under 5 seconds (adjusted for realistic performance)
        assert (
            metrics["memory_delta_mb"] < 100
        )  # Should not use more than 100MB additional memory (adjusted)
    ...
    async def test_concurrent_generation_performance(self, mock_generation_engine):
        ...
        requests = [
            GenerationRequest(software_name=f"software-{i}", llm_provider=LLMProvider.OPENAI)
            for i in range(10)
        ]

        # Execute concurrently
        tasks = [mock_generation_engine.generate_saidata(request) for request in requests]
...
class TestBatchEnginePerformance:
    ...
    def mock_batch_engine(self):
        ...
        config = {"batch": {"max_concurrent": 5, "retry_attempts": 1, "timeout": 30}}
    ...
    async def test_indexing_performance(self, temp_index_dir, large_package_dataset):
        ...
        with patch("saigen.repositories.indexer.RAG_AVAILABLE", True):
            ...
            mock_model.encode.return_value = [
                [0.1] * 384 for _ in range(len(large_package_dataset))
            ]

            with patch.object(indexer, "_get_model", return_value=mock_model), patch(
                "saigen.repositories.indexer.faiss"
            ) as mock_faiss:
                mock_index = Mock()
        ...
        with patch("saigen.core.generation_engine.OpenAIProvider") as mock_provider_class:
            ...
            for i in range(100):
                request = GenerationRequest(
                    software_name=f"test-{i}", llm_provider=LLMProvider.OPENAI
                )
