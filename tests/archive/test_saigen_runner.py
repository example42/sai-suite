...
class SaigenTestRunner:
    ...
    def run_specific_component_tests(self, component: str, verbose: bool = True) -> Dict[str, Any]:
        ...
        if component not in component_test_map:
            return {
                "success": False,
                "error": f"Unknown component: {component}. Available: {
                    list(
                        component_test_map.keys())}",
            }
    ...
    def generate_report(self, output_file: Optional[Path] = None) -> str:
        ...
        for category, result in self.results.items():
            ...
            if result.get("stdout"):
                report_lines.extend(["### Output", "```", result["stdout"], "```", ""])

            if result.get("stderr"):
                report_lines.extend(["### Errors", "```", result["stderr"], "```", ""])
...
def main():
    ...
    try:
        if args.command == "all":
            ...
            results = {
                "unit": runner.run_unit_tests(verbose=args.verbose, coverage=not args.no_coverage)
            }
            ...
            if not args.component:
                ...
                sys.exit(1)
            results = {
                "component": runner.run_specific_component_tests(
                    args.component, verbose=args.verbose
                )
            }
