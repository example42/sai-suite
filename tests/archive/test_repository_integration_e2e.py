...
class TestRepositoryIntegrationE2E:
    ...
    def test_complete_tarball_repository_workflow(self):
        ...
        with patch("urllib.request.urlopen") as mock_urlopen:
            ...
            result = self.tarball_handler.get_latest_release_info(
                TEST_RELEASE_URLS["small_release"]
            )
    ...
    def test_repository_manager_end_to_end_workflow(self):
        ...
        with patch.object(
            self.repository_manager, "_clone_or_update_repository", side_effect=mock_clone_or_update
        ):
...
class TestRepositoryOfflineModeIntegration:
    ...
    def create_cached_repository(self):
        ...
        saidata = {
            "version": "0.2",
            "metadata": {"name": "nginx", "display_name": "Nginx Web Server"},
            "packages": [{"name": "nginx"}],
        }
    ...
    def test_stale_cache_warning(self):
        ...
        self.repository_manager.repository_cache._update_repository_timestamp(
            self.config.saidata_repository_url, self.config.saidata_repository_branch, old_time
        )
...
-----END OPENSSH PRIVATE KEY-----"""
    ...
    def test_ssh_authentication_setup(self):
        ...
        auth_config = {"type": "ssh", "ssh_key_path": str(ssh_key_path), "ssh_user": "git"}
    ...
    def test_token_authentication_setup(self):
        ...
        auth_config = {"type": "token", "token": "ghp_test_token_123456789"}

        # Test token authentication setup
        url_with_auth = self.git_handler._setup_token_authentication(
            "https://github.com/user/repo.git", auth_config
        )
    ...
    def test_authentication_failure_handling(self):
        ...
        with patch.object(self.git_handler, "_execute_git_command") as mock_execute:
            mock_execute.return_value = GitOperationResult(
                success=False, stderr="fatal: Authentication failed", exit_code=128
            )

            result = self.git_handler.clone_repository(
                TEST_REPOSITORIES["private"], self.temp_dir / "private_repo"
            )
    ...
    @pytest.mark.skipif(
        not os.getenv("TEST_SSH_KEY"), reason="SSH key not provided for integration test"
    )
    def test_real_ssh_authentication(self):
        ...
        auth_config = {"type": "ssh", "ssh_key_path": ssh_key_path, "ssh_user": "git"}

        clone_path = self.temp_dir / "ssh_cloned_repo"

        # This will likely fail without proper SSH setup, but tests the workflow
        result = self.git_handler.clone_repository_with_auth(test_repo_url, clone_path, auth_config)
...
class TestRepositoryPerformanceIntegration:
    ...
    def setup_method(self):
        ...
        self.config = SaiConfig(
            cache_directory=self.cache_dir, saidata_repository_cache_dir=self.repo_dir
        )
    ...
    def test_large_repository_cloning_performance(self):
        ...
        assert (
            len(software_dirs) == 500
        ), f"Expected 500 software directories, found {len(software_dirs)}"
    ...
    def test_large_repository_saidata_loading_performance(self):
        ...
        assert loaded_count == len(
            software_names
        ), f"Expected {len(software_names)} loaded, got {loaded_count}"
        ...
        ), f"Average loading time too high: {avg_time_per_file:.3f} seconds"
    ...
    def test_concurrent_repository_operations_performance(self):
        ...
        def clone_repository(repo_path, clone_path):
            ...
            clone_results.append(
                {"success": result.success, "time": end_time - start_time, "repo": repo_path.name}
            )
        ...
        assert (
            successful_clones == repo_count
        ), f"Expected {repo_count} successful clones, got {successful_clones}"
...
class TestRepositorySecurityIntegration:
    ...
    def test_path_traversal_protection(self):
        ...
        for malicious_path in malicious_paths:
            ...
            assert not sanitized.startswith(
                "/etc/"
            ), f"Path traversal not prevented: {malicious_path}"
            assert not sanitized.startswith(
                "C:\\Windows"
            ), f"Path traversal not prevented: {malicious_path}"
    ...
    def test_repository_integrity_validation(self):
        ...
        self.git_handler._validate_repository_structure(test_repo)
