...
class TestApplyCommand:
    ...
    def test_apply_with_parallel_option(self):
        ...
        try:
            result = self.runner.invoke(
                cli, ["apply", str(temp_file), "--parallel", "--yes", "--dry-run"]
            )
    ...
    def test_apply_with_continue_on_error_option(self):
        ...
        try:
            result = self.runner.invoke(
                cli, ["apply", str(temp_file), "--continue-on-error", "--yes", "--dry-run"]
            )
    ...
    def test_apply_with_timeout_option(self):
        ...
        try:
            result = self.runner.invoke(
                cli, ["apply", str(temp_file), "--timeout", "300", "--yes", "--dry-run"]
            )
    ...
    def test_apply_complex_action_file(self):
        ...
        complex_data = {
            "config": {"verbose": True, "dry_run": True, "continue_on_error": True},
            "actions": {
    ...
    def test_apply_global_options_override(self):
        ...
        try:
            ...
            result = self.runner.invoke(
                cli, ["--verbose", "--dry-run", "apply", str(temp_file), "--yes"]
            )
