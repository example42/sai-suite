...
from sai.core.git_repository_handler import GitOperationResult, GitRepositoryHandler, RepositoryInfo
...
class TestGitRepositoryHandler:
    ...
    def test_clone_repository_git_unavailable(self, mock_git_available):
        ...
        result = self.handler.clone_repository(
            "https://github.com/example/repo.git", self.temp_dir / "repo"
        )
    ...
    def test_update_repository_success(
        self, mock_verify, mock_repo_info, mock_execute, mock_execute_retry, mock_git_available
    ):
        ...
        mock_repo_info.side_effect = [
            RepositoryInfo(
                url="https://github.com/test/repo", branch="main", commit_hash="abc123"
            ),  # before update
            RepositoryInfo(
                url="https://github.com/test/repo", branch="main", commit_hash="def456"
            ),  # after update
        ]
        ...
        mock_execute.side_effect = [
            GitOperationResult(
                success=True, message="Branch detected", stdout="main\n"
            ),  # branch detection
            GitOperationResult(success=True, message="Status check", stdout=""),  # status check
            GitOperationResult(
                success=True, message="Reset successful", stdout="HEAD is now at def456\n"
            ),  # reset
        ]
    ...
    def test_get_repository_info_success(self, mock_execute):
        ...
        mock_execute.side_effect = [
            GitOperationResult(
                success=True, message="Remote URL", stdout="https://github.com/example/repo.git\n"
            ),  # remote URL
            GitOperationResult(success=True, message="Branch", stdout="main\n"),  # branch
            GitOperationResult(
                success=True, message="Commit hash", stdout="abc123def456\n"
            ),  # commit hash
            GitOperationResult(success=True, message="Status", stdout=""),  # status (clean)
            GitOperationResult(
                success=True, message="Last commit", stdout="2023-12-01 10:30:00 +0000\n"
            ),  # last commit date
        ]
    ...
    def test_setup_git_environment_ssh_auth(self):
        ...
        env = self.handler._setup_git_environment(
            RepositoryAuthType.SSH, {"ssh_key_path": str(ssh_key_path)}
        )
    ...
    def test_setup_git_environment_token_auth(self):
        ...
        env = self.handler._setup_git_environment(
            RepositoryAuthType.TOKEN, {"token": "ghp_1234567890", "username": "user"}
        )
    ...
    def test_setup_git_environment_basic_auth(self):
        ...
        env = self.handler._setup_git_environment(
            RepositoryAuthType.BASIC, {"username": "user", "password": "pass"}
        )
