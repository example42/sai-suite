...
from sai.models.actions import ActionConfig, ActionFile, ActionItem, Actions
...
class TestActionItem:
    ...
    def test_action_item_with_extra_params(self):
        ...
        item = ActionItem(
            name="nginx", provider="apt", version="1.20", force=True, config_file="/etc/nginx.conf"
        )
        assert item.name == "nginx"
        assert item.provider == "apt"
...
class TestActions:
    ...
    def test_install_actions_mixed(self):
        ...
        actions = Actions(install=["nginx", {"name": "docker", "provider": "apt"}])
    ...
    def test_multiple_action_types(self):
        ...
        actions = Actions(install=["nginx"], start=["nginx"], uninstall=["old-package"])
        assert actions.has_actions()
...
class TestActionFile:
    ...
    def test_flexible_action_types(self):
        ...
        actions = Actions(
            install=["nginx"], deploy=["app1", "app2"], backup=["database"], custom_action=["item1"]
        )
