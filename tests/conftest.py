...
def sample_saidata():
    ...
    return SaiData(
        ...
        packages=[Package(name="nginx", version=">=1.18.0")],
...
def test_saidata_files(temp_dir):
    ...
    return {"nginx": nginx_file, "git": git_file, "directory": saidata_dir}
...
def test_provider_files(temp_dir):
    ...
    brew_data = {
        ...
        "actions": {
            ...
            "search": {"template": "brew search {{search_term}}", "requires_sudo": False},
        },
    }
    ...
    return {"apt": apt_file, "brew": brew_file, "directory": provider_dir}
...
def mock_system_commands():
    ...
    with pytest.mock.patch(
        ...
        "sai.utils.system.get_executable_path", return_value=Path("/usr/bin/test")
    ), pytest.mock.patch(
        "sai.utils.system.get_executable_version", return_value="test 1.0.0"
    ):
        yield
...
def sample_saigen_config(temp_dir):
    ...
    return SaigenConfig(
        llm_providers={
            "openai": LLMConfig(
                api_key="test-key", model="gpt-4o-mini", max_tokens=2000, temperature=0.1
            )
        },
        output_directory=str(temp_dir / "output"),
        cache_directory=str(temp_dir / "cache"),
        generation={"retry_attempts": 2, "timeout": 60, "use_rag": True},
    )
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "llm_integration: mark test as requiring LLM API access")
...
def pytest_addoption(parser):
    ...
    parser.addoption(
        "--no-integration", action="store_true", default=False, help="Skip integration tests"
    )
    parser.addoption(
        "--no-system",
        action="store_true",
    ...
    parser.addoption(
        "--no-performance", action="store_true", default=False, help="Skip performance tests"
    )
    parser.addoption(
        "--no-llm",
        action="store_true",
