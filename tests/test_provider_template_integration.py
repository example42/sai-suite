...
from sai.models.provider_data import (
    ProviderData,
    Provider,
    Action,
    Step,
...
class TestProviderTemplateIntegration:
    ...
    def test_resolve_single_template(self):
        ...
        result = provider.resolve_template(
            "systemctl status {{saidata.services.*.service_name}}",
            saidata
        )
    ...
    def test_resolve_template_with_additional_context(self):
        ...
        result = provider.resolve_template(
            "Configure {{name}} on port {{port}} using {{config_file}}",
            saidata,
            additional_context
        )
    ...
    def test_complex_template_scenario(self):
        ...
        assert result["steps"][1]["command"] == "apt-get install -y postgresql-13 postgresql-client-13 postgresql-contrib-13"
        assert result["steps"][2]["command"] == "sudo -u postgres initdb -D /var/lib/postgresql/13.7/main"
        assert result["steps"][3]["command"] == "systemctl start postgresql && systemctl enable postgresql"
