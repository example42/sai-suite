...
class TestTemplateIntegration:
    ...
    def test_nginx_install_template(self):
        ...
        result = engine.resolve_template("apt-get install -y {{saidata.packages.*.name}}", saidata)
        ...
        result = engine.resolve_template(
            "Installing {{display_name}} ({{name}}) version {{version}}", saidata
        )
    ...
    def test_error_handling_with_realistic_templates(self):
        ...
        with pytest.raises(TemplateResolutionError):
            engine.resolve_template("systemctl start {{undefined_variable}}", saidata)

        # Test malformed template
        with pytest.raises(TemplateResolutionError):
            engine.resolve_template(
                "install {{saidata.packages.*.name", saidata  # Missing closing brace
            )
