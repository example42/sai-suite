...
from saigen.models.generation import LLMProvider
...
def sample_existing_saidata():
    ...
    return SaiData(
        ...
        providers={"apt": {"packages": [{"name": "nginx", "version": "1.20.0"}]}},
    )
...
def sample_fresh_saidata():
    ...
    return SaiData(
        ...
        providers={
            "apt": {"packages": [{"name": "nginx", "version": "1.22.0"}]},
            "brew": {"packages": [{"name": "nginx", "version": "1.22.0"}]},
        },
        ports=[
  ...
  - port: 443
    ...
    service: https
        ...
        mock_provider.generate_saidata.return_value = Mock(
            content=fresh_yaml_content, tokens_used=1500, cost_estimate=0.03
        )
...
ports:
  ...
  - port: 443
    ...
    service: https
        ...
        mock_provider = AsyncMock()
        mock_provider.generate_saidata.return_value = Mock(
            content=fresh_yaml_content, tokens_used=1200, cost_estimate=0.024
        )
        ...
        assert (
            result.saidata.metadata.description == "High-performance web server"
        )  # Original preserved
    ...
    async def test_file_based_update_workflow(self, sample_existing_saidata):
        ...
        try:
            ...
            assert (
                loaded_saidata.metadata.description == sample_existing_saidata.metadata.description
            )
    ...
    async def test_error_handling_in_update(self, sample_existing_saidata):
        ...
        assert "LLM generation failed" in str(
            result.validation_errors[0].message
        ) or "Failed to generate fresh saidata" in str(result.validation_errors[0].message)
