...
class TestGitRepositoryErrorHandling:
    ...
    def test_network_error_handling(self, handler):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            mock_run.side_effect = subprocess.CalledProcessError(
                128, ['git', 'clone'], stderr="fatal: unable to access 'https://github.com/test/repo.git/': Could not resolve host"
            )
    ...
    def test_corrupted_repository_handling(self, handler, temp_dir):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.side_effect = subprocess.CalledProcessError(
                128, ['git', 'status'], stderr="fatal: not a git repository (or any of the parent directories): .git"
            )
