...
class TestRepositoryManager:
    ...
    async def test_get_packages_from_cache(self, repository_manager, mock_cache, sample_packages):
        ...
        with patch.object(repository_manager.universal_manager, 'get_packages', return_value=sample_packages):
    ...
    async def test_search_packages(self, repository_manager, mock_cache, sample_packages):
        ...
        with patch.object(repository_manager.universal_manager, 'search_packages', return_value=mock_search_result):
    ...
    async def test_get_package_info(self, repository_manager, mock_cache, sample_packages):
        ...
        with patch.object(repository_manager.universal_manager, 'get_package_details', return_value=expected_package):
    ...
    async def test_get_package_info_not_found(self, repository_manager, mock_cache):
        ...
        with patch.object(repository_manager.universal_manager, 'get_package_details', return_value=None):
    ...
    async def test_update_cache(self, repository_manager, mock_cache, sample_packages):
        ...
        with patch.object(repository_manager.universal_manager, 'update_cache', return_value=mock_result):
    ...
    async def test_downloader_error_handling(self, repository_manager, mock_cache):
        ...
        with patch.object(repository_manager.universal_manager, 'get_packages', side_effect=RepositoryError("Download failed")):
            with pytest.raises(RepositoryError):
...
class TestRepositoryManagerIntegration:
    ...
    async def test_full_repository_workflow(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(manager.universal_manager, 'update_cache', return_value={"test_repo": True}):
                with patch.object(manager.universal_manager, 'get_packages', return_value=mock_packages):
                    with patch.object(manager.universal_manager, 'search_packages', return_value=SearchResult(
                        query="test", packages=mock_packages, total_results=1, search_time=0.1, repository_sources=["test_repo"]
                    )):
                        # Test cache update
