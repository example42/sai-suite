...
class TestRepositoryManager:
    ...
    def create_local_test_repository(
        self, name: str, saidata_count: int = 10
        ...
        subprocess.run(
            ["git", "commit", "-m", "Initial commit with test saidata"],
            cwd=repo_path,
            check=True,
            capture_output=True,
        )
    ...
    def create_large_test_repository(
        self, name: str, software_count: int = 1000
    ) -> TestRepositoryConfig:
        ...
        for i in range(software_count):
            ...
            if (i + 1) % 100 == 0:
                subprocess.run(["git", "add", "."], cwd=repo_path, check=True, capture_output=True)
                subprocess.run(
                    ["git", "commit", "-m", f"Add software packages {i - 99:04d}-{i:04d}"],
                    cwd=repo_path,
                    check=True,
                    capture_output=True,
                )
        ...
        subprocess.run(["git", "add", "."], cwd=repo_path, check=True, capture_output=True)
        subprocess.run(
            ["git", "commit", "-m", "Final commit"], cwd=repo_path, check=True, capture_output=True
        )
    ...
    def create_malformed_repository(self, name: str) -> TestRepositoryConfig:
        ...
        subprocess.run(["git", "add", "."], cwd=repo_path, check=True, capture_output=True)
        subprocess.run(
            ["git", "commit", "-m", "Add malformed saidata"],
            cwd=repo_path,
            check=True,
            capture_output=True,
        )
