...
from saigen.core.advanced_validator import (
    AdvancedSaidataValidator, QualityMetric, QualityScore, QualityReport
)
...
class TestAdvancedSaidataValidator:
    ...
    @pytest.mark.asyncio
    async def test_validate_comprehensive_no_repository_check(self, advanced_validator, sample_saidata):
    ...
    async def test_assess_completeness_high_score(self, advanced_validator):
        ...
        saidata = SaiData(
            ...
            providers={
                "apt": ProviderConfig(packages=[Package(name="nginx")]),
                "dnf": ProviderConfig(packages=[Package(name="nginx")]),
    ...
    async def test_assess_completeness_low_score(self, advanced_validator):
        ...
        saidata = SaiData(
            version="0.2",
            metadata=Metadata(name="test")  # Minimal metadata
        )
    ...
    async def test_assess_cross_reference_integrity_valid(self, advanced_validator):
        ...
        saidata = SaiData(
            ...
            providers={
                "apt": ProviderConfig(
                    repositories=[
                        Repository(name="main-repo", url="http://example.com")
                    ],
                    packages=[
                        Package(name="test-pkg", repository="main-repo")
                    ]
                )
            }
        )
    ...
    async def test_assess_cross_reference_integrity_invalid(self, advanced_validator):
        ...
        saidata = SaiData(
            ...
            providers={
                "apt": ProviderConfig(
                    packages=[
                        Package(name="test-pkg", repository="undefined-repo")
                    ]
                )
            }
        )
    ...
    async def test_package_exists_in_repository_exception(self, advanced_validator):
        ...
        advanced_validator.repository_manager.search_packages.side_effect = Exception("Network error")
    ...
    def test_detect_orphaned_configurations(self, advanced_validator):
        ...
        from saigen.models.saidata import Service, File
    ...
    def test_calculate_overall_score(self, advanced_validator):
        ...
        metric_scores = {
            QualityMetric.COMPLETENESS: QualityScore(
                ...
                score=0.8, max_score=1.0, details={}, issues=[], suggestions=[]
            ),
            QualityMetric.ACCURACY: QualityScore(
                metric=QualityMetric.ACCURACY,
                score=0.9, max_score=1.0, details={}, issues=[], suggestions=[]
            ),
            QualityMetric.CONSISTENCY: QualityScore(
                metric=QualityMetric.CONSISTENCY,
                score=0.7, max_score=1.0, details={}, issues=[], suggestions=[]
            )
        }
...
class TestQualityMetrics:
    ...
    async def test_completeness_metric_comprehensive(self):
        ...
        from saigen.models.saidata import Urls, SecurityMetadata
        ...
        saidata = SaiData(
            ...
            metadata=Metadata(
                ...
                security=SecurityMetadata(
                    security_contact="security@nginx.org"
                )
            ),
            providers={
                "apt": ProviderConfig(packages=[Package(name="nginx")]),
