...
from saigen.llm.providers.base import LLMResponse, ModelCapability, ModelInfo
...
class TestGenerationEngine:
    ...
    def mock_config(self):
        ...
        return {"llm_providers": {"openai": {"api_key": "test-api-key", "model": "gpt-4o-mini"}}}
...
"""
    ...
    @pytest.mark.asyncio
    async def test_generate_saidata_success(
        self, generation_engine, sample_request, sample_saidata_yaml
    ):
    ...
    async def test_generate_saidata_invalid_yaml(self, generation_engine, sample_request):
        ...
        mock_llm_response = LLMResponse(
            content="invalid: yaml: content: [unclosed", tokens_used=500, cost_estimate=0.001
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_saidata_schema_validation_failure(
        self, generation_engine, sample_request
    ):
...
"""
        ...
        mock_llm_response = LLMResponse(content=invalid_yaml, tokens_used=300, cost_estimate=0.0006)
    ...
    @pytest.mark.asyncio
    async def test_generate_saidata_retry_on_validation_failure(
        self, generation_engine, sample_request, sample_saidata_yaml
    ):
...
"""
        ...
        mock_provider.generate_saidata = AsyncMock(
            side_effect=[
...
"""
    ...
    def test_validate_request_empty_software_name(self, generation_engine):
        ...
        request = GenerationRequest(software_name="", llm_provider=LLMProvider.OPENAI)
    ...
    def test_validate_request_unavailable_provider(self, generation_engine):
        ...
        request = GenerationRequest(
            software_name="nginx", llm_provider=LLMProvider.ANTHROPIC  # Not configured
        )
    ...
    async def test_save_saidata(self, generation_engine, tmp_path):
        ...
        saidata = SaiData(
            version="0.2", metadata=Metadata(name="test-software", description="Test software")
        )
    ...
    async def test_validate_saidata_file(self, generation_engine, tmp_path):
        ...
        test_data = {
            "version": "0.2",
            "metadata": {"name": "test-software", "description": "Test software"},
        }
    ...
    def test_format_validation_report(self, generation_engine):
        ...
        from saigen.core.validator import ValidationError, ValidationResult, ValidationSeverity
...
async def test_generation_engine_integration():
    ...
    config = {"llm_providers": {"openai": {"api_key": "test-key", "model": "gpt-4o-mini"}}}
...
"""
    ...
    with patch("saigen.core.generation_engine.OpenAIProvider") as mock_provider_class:
        ...
        mock_provider.get_provider_name.return_value = "openai"
        mock_provider.generate_saidata = AsyncMock(
            return_value=LLMResponse(content=sample_yaml, tokens_used=800, cost_estimate=0.0016)
        )
        mock_provider_class.return_value = mock_provider
        ...
        request = GenerationRequest(software_name="redis", llm_provider=LLMProvider.OPENAI)
