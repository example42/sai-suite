...
class TestBatchGenerationEngine:
    ...
    def sample_saidata(self):
        ...
        return SaiData(
            version="0.2",
            metadata=Metadata(name="nginx", description="Web server"),
            providers={"apt": {"packages": [{"name": "nginx", "package_name": "nginx"}]}},
        )

    @pytest.mark.asyncio
    async def test_generate_batch_success(
        self, batch_engine, mock_generation_engine, sample_saidata
    ):
...
"""
    ...
    @pytest.mark.asyncio
    async def test_generate_with_output_directory(
        self, batch_engine, mock_generation_engine, tmp_path, sample_saidata
    ):
    ...
    async def test_empty_software_list_error(self, batch_engine):
        ...
        request = BatchGenerationRequest(
            software_list=[], target_providers=["apt"], llm_provider=LLMProvider.OPENAI
        )
