...
class TestSaidataValidator:
    ...
    def valid_saidata(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "packages": [
                {"name": "test-package", "package_name": "test-package", "version": "1.0.0"}
            ],
            "providers": {
                "apt": {
                    "packages": [
    ...
    def test_validate_invalid_version_format(self, validator: SaidataValidator):
        ...
        invalid_data = {"version": "invalid-version", "metadata": {"name": "test"}}
    ...
    def test_validate_invalid_enum_value(
        self, validator: SaidataValidator, valid_saidata: Dict[str, Any]
    ):
        """Test validation with invalid enum values."""
        valid_saidata["services"] = [
            {"name": "test-service", "type": "invalid-service-type"}  # Invalid enum value
        ]
        ...
        enum_errors = [
            ...
            if "enum" in error.code or "one of" in error.message.lower()
        ]
        assert len(enum_errors) > 0

    def test_validate_invalid_port_range(
        self, validator: SaidataValidator, valid_saidata: Dict[str, Any]
    ):
        """Test validation with invalid port numbers."""
        valid_saidata["ports"] = [{"port": 70000}]  # Invalid port number
        ...
        port_warnings = [
            warning for warning in result.warnings if "port" in warning.message.lower()
        ]
        assert len(port_warnings) > 0

    def test_validate_privileged_port_warning(
        self, validator: SaidataValidator, valid_saidata: Dict[str, Any]
    ):
        """Test warning for privileged ports."""
        valid_saidata["ports"] = [{"port": 80}]  # Privileged port
        ...
        privileged_warnings = [
            warning for warning in result.warnings if "privileged" in warning.message.lower()
        ]
        assert len(privileged_warnings) > 0

    def test_validate_suspicious_package_name(
        self, validator: SaidataValidator, valid_saidata: Dict[str, Any]
    ):
        ...
        package_warnings = [
            warning for warning in result.warnings if "package name" in warning.message.lower()
        ]
    ...
    def test_validate_cross_references(
        ...
        cross_ref_warnings = [
            ...
            if "undefined repository" in warning.message.lower()
        ]
    ...
    def test_validate_file_paths(self, validator: SaidataValidator, valid_saidata: Dict[str, Any]):
        ...
        valid_saidata["files"] = [
            {"name": "config", "path": "~/relative/path"},  # Tilde expansion
            {"name": "data", "path": "relative/path"},  # Relative path
        ]
        ...
        path_warnings = [
            warning for warning in result.warnings if "path" in warning.message.lower()
        ]
    ...
    def test_validate_pydantic_model(self, validator: SaidataValidator):
        ...
        saidata = SaiData(version="0.2", metadata=Metadata(name="test-software"))
    ...
    def test_validate_file_not_found(self, validator: SaidataValidator):
        ...
        not_found_errors = [
            error for error in result.errors if "not found" in error.message.lower()
        ]
    ...
    def test_format_validation_report_with_context(self, validator: SaidataValidator):
        ...
        invalid_data = {
            "version": "0.2",
            "metadata": {"name": "test"},
            "services": [{"name": "test-service", "type": "invalid-type"}],
        }
    ...
    def test_validation_error_properties(self, validator: SaidataValidator):
        ...
        invalid_data = {
            "version": "invalid",
            "metadata": {"name": "test"},
            "ports": [{"port": 80}],  # Privileged port (warning)
        }
...
class TestValidationErrorFormatting:
    ...
    def test_type_error_formatting(self, validator: SaidataValidator):
        ...
        invalid_data = {"version": 123, "metadata": {"name": "test"}}  # Should be string
    ...
    def test_enum_error_formatting(self, validator: SaidataValidator):
        ...
        invalid_data = {
            "version": "0.2",
            "metadata": {"name": "test"},
            "services": [{"name": "test-service", "type": "invalid-enum-value"}],
        }

        result = validator.validate_data(invalid_data)

        enum_errors = [
            error for error in result.errors if "must be one of" in error.message.lower()
        ]
        assert len(enum_errors) > 0
