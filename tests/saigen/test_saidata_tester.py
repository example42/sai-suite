...
def sample_saidata():
    ...
    return SaiData(
        ...
        providers={
            "apt": ProviderConfig(
                packages=[
                    Package(name="test-package", package_name="test-package", version="1.0.0")
                ]
            ),
            "brew": ProviderConfig(
                packages=[Package(name="test-formula", package_name="test-formula")]
            ),
        },
    )
...
class TestSaidataTester:
    ...
    async def test_test_file_success(self, tester, sample_saidata_file):
        ...
        with patch.object(tester, "_test_dry_run", new_callable=AsyncMock) as mock_dry_run:
            ...
            with patch.object(
                tester, "_test_provider_compatibility", new_callable=AsyncMock
            ) as mock_compat:
