...
def sample_fresh_saidata():
    ...
    return SaiData(
        ...
        providers={
            "apt": {"packages": [{"name": "nginx", "package_name": "nginx", "version": "1.22.0"}]},
            "brew": {"packages": [{"name": "nginx", "package_name": "nginx", "version": "1.22.0"}]},
        },
        ports=[
...
class TestUpdateEngine:
    ...
    @pytest.mark.asyncio
    async def test_update_saidata_force_update(
        self, update_engine, sample_existing_saidata, sample_fresh_saidata, mock_generation_engine
    ):
    ...
    @pytest.mark.asyncio
    async def test_merge_preserve_strategy(
        self, update_engine, sample_existing_saidata, sample_fresh_saidata
    ):
    ...
    @pytest.mark.asyncio
    async def test_merge_enhance_strategy(
        self, update_engine, sample_existing_saidata, sample_fresh_saidata
    ):
    ...
    @pytest.mark.asyncio
    async def test_merge_replace_strategy(
        self, update_engine, sample_existing_saidata, sample_fresh_saidata
    ):
        ...
        assert (
            merged_dict["metadata"]["description"]
    ...
    async def test_merge_metadata_enhance(self, update_engine):
        ...
        fresh_metadata = {
            ...
            "maintainer": "NGINX Team",
            "urls": {"website": "https://nginx.org", "documentation": "https://nginx.org/en/docs/"},
        }

        stats = await update_engine._merge_metadata_enhance(
            existing_metadata, fresh_metadata, False
        )
    ...
    async def test_merge_providers_enhance(self, update_engine):
        ...
        fresh_providers = {
            "apt": {
                "packages": [{"name": "nginx", "package_name": "nginx", "version": "1.22.0"}],
                "services": [{"name": "nginx", "enabled": True}],
            },
            "brew": {"packages": [{"name": "nginx", "package_name": "nginx", "version": "1.22.0"}]},
        }

        stats = await update_engine._merge_providers_enhance(
            existing_providers, fresh_providers, False
        )
    ...
    def test_get_nested_field(self, update_engine):
        ...
        data = {"metadata": {"name": "nginx", "urls": {"website": "https://nginx.org"}}}
    ...
    @pytest.mark.asyncio
    async def test_update_saidata_generation_failure(
        self, update_engine, sample_existing_saidata, mock_generation_engine
    ):
        ...
        result = await update_engine.update_saidata(
            existing_saidata=sample_existing_saidata, llm_provider=LLMProvider.OPENAI
        )
    ...
    @pytest.mark.asyncio
    async def test_update_saidata_with_user_hints(
        self, update_engine, sample_existing_saidata, sample_fresh_saidata, mock_generation_engine
    ):
        """Test update with user hints."""
        user_hints = {"focus_on": "security", "add_monitoring": True}
