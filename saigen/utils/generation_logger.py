...
class GenerationLogger:
    ...
    def update_process_step(
        ...
        for logged_step in self.log_data["process_steps"]:
            if (
                logged_step["step_name"] == step.step_name
                and logged_step["timestamp"] == step.timestamp
            ):
    ...
    def log_final_result(
        ...
        if saidata:
            result_data["saidata_summary"] = {
                ...
                "providers": list(saidata.providers.keys()) if saidata.providers else [],
                "package_count": sum(
                    len(p.packages) for p in saidata.providers.values() if p.packages
                )
                if saidata.providers
                else 0,
                "service_count": sum(
                    len(p.services) for p in saidata.providers.values() if p.services
                )
                if saidata.providers
                else 0,
            }
    ...
    @contextmanager
    def log_data_op(
        self, operation_type: str, description: str, input_data: Optional[Dict[str, Any]] = None
    ):
        ...
        try:
            ...
            self.log_data_operation(
                operation_type, description, input_data, output_data, duration, True
            )
        except Exception as e:
            duration = time.time() - start_time
            self.log_data_operation(
                operation_type, description, input_data, None, duration, False, str(e)
            )
            raise
        finally:
