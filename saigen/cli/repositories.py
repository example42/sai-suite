...
def get_repository_manager(cache_dir: Optional[str], config_dir: Optional[str]) -> RepositoryManager:
...
async def _list_repositories(platform: Optional[str], repo_type: Optional[str], 
            ...
            if output_format == 'json':
                ...
                for repo in repositories:
                    rows.append([
                        ...
                        (repo.description or '')[:50] + ('...' if len(repo.description or '') > 50 else '')
                    ])
...
async def _search_packages(query: str, platform: Optional[str], repo_type: Optional[str],
                         limit: int, output_format: str, cache_dir: Optional[str],
                         config_dir: Optional[str]):
    ...
    try:
        ...
        async with manager:
            ...
            if output_format == 'json':
                ...
                click.echo(f"Supported platforms: {', '.join(sorted(stats.get('supported_platforms', [])))}")
                click.echo(f"Supported types: {', '.join(sorted(stats.get('supported_types', [])))}")
...
@click.option('--cache-dir', help='Cache directory path')
@click.option('--config-dir', help='Configuration directory path')
def update_cache(repository: Optional[str], force: bool, cache_dir: Optional[str], 
                config_dir: Optional[str]):
    """Update repository cache."""
    asyncio.run(_update_cache(repository, force, cache_dir, config_dir))


async def _update_cache(repository: Optional[str], force: bool, cache_dir: Optional[str],
                       config_dir: Optional[str]):
...
@click.option('--cache-dir', help='Cache directory path')
@click.option('--config-dir', help='Configuration directory path')
def info(package_name: str, version: Optional[str], platform: Optional[str],
         output_format: str, cache_dir: Optional[str], config_dir: Optional[str]):
    """Get detailed information about a package."""
    asyncio.run(_get_package_info(package_name, version, platform, output_format, 
                                cache_dir, config_dir))


async def _get_package_info(package_name: str, version: Optional[str], 
                          platform: Optional[str], output_format: str,
                          cache_dir: Optional[str], config_dir: Optional[str]):
        ...
        async with manager:
            ...
            if output_format == 'json':
                ...
                info_data = [
                    ...
                    ['Last Updated', package.last_updated.strftime('%Y-%m-%d %H:%M:%S') if package.last_updated else 'N/A']
                ]
