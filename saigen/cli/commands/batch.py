...
@click.option(
    "--category-filter", "-c", help='Filter by category using regex pattern (e.g., "web|database")'
)
@click.option(
    "--max-concurrent",
    "-j",
...
@click.option("--preview", is_flag=True, help="Preview what would be processed without generating")
@click.pass_context
def batch(
    ctx: click.Context,
