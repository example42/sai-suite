...
@click.option(
    '--no-repository-check',
    is_flag=True,
    help='Skip repository accuracy checking'
)
@click.option(
    '--format',
    'output_format',
...
def quality(
    ...
    """
    try:
        asyncio.run(_run_quality_assessment(
            file_path, metric, threshold, not no_repository_check, 
...
def _format_csv_output(quality_report, file_path: Path) -> str:
    ...
    # Overall score
    lines.append(f"{file_path},overall,{quality_report.overall_score:.3f},1.000,0,{len(quality_report.recommendations)}")
...
def _format_text_output(quality_report, file_path: Path, threshold: float, 
                       metric_filter: Optional[str]) -> str:
    ...
    lines.append(f"Assessment Time: {quality_report.generated_at.strftime('%Y-%m-%d %H:%M:%S UTC')}")
    lines.append("")
    ...
    if not metric_filter:
        ...
        lines.append(f"Overall Quality Score: {score_emoji} {quality_report.overall_score:.3f}/1.000 ({status})")
    ...
    if not metric_filter:
        ...
        lines.append("-" * 40)
        lines.append(f"Schema Validation: {'✅ PASSED' if not quality_report.validation_result.has_errors else '❌ FAILED'}")
        lines.append(f"Quality Threshold: {'✅ PASSED' if quality_report.overall_score >= threshold else '❌ FAILED'}")
