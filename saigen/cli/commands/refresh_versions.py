...
@click.option('--providers', multiple=True,
              help='Target specific providers (e.g., apt, brew, winget)')
...
@click.option('--check-only', is_flag=True,
              help='Check for version updates without modifying files')
@click.option('--show-unchanged', is_flag=True,
              help='Show packages that are already up-to-date')
@click.option('--use-cache/--no-cache', default=True,
              help='Use cached repository data (default: enabled)')
@click.pass_context
def refresh_versions(ctx: click.Context, saidata_file: Path, output: Optional[Path],
                    providers: tuple, backup: bool, backup_dir: Optional[Path],
                    check_only: bool, show_unchanged: bool, use_cache: bool):
...
async def _refresh_versions(saidata: SaiData, config: Any,
                           target_providers: Optional[List[str]],
                           use_cache: bool, verbose: bool) -> VersionRefreshResult:
    ...
    for pkg_info in packages_to_check:
        try:
            ...
            result.errors.append(
                f"Failed to check {pkg_info['package_name']} "
                f"({pkg_info['provider']}): {str(e)}"
            )
...
def _collect_packages_from_saidata(saidata: SaiData,
                                   target_providers: Optional[List[str]]) -> List[Dict[str, Any]]:
                        ...
                        for pkg in pkg_source.packages:
                            if pkg.version:
                                packages.append({
                                    ...
                                    'package_name': pkg.package_name,
                                    'current_version': pkg.version,
                                    'location': f'providers.{provider_name}.package_sources.{pkg_source.name}',
                                    'object': pkg
                                })
                        ...
                        for pkg in repo.packages:
                            if pkg.version:
                                packages.append({
                                    ...
                                    'package_name': pkg.package_name,
                                    'current_version': pkg.version,
                                    'location': f'providers.{provider_name}.repositories.{repo.name}',
                                    'object': pkg
                                })
...
async def _query_package_version(repo_manager: RepositoryManager,
                                 package_name: str, provider: str,
                                 use_cache: bool, verbose: bool) -> Optional[str]:
...
def _update_package_version(saidata: SaiData, pkg_info: Dict[str, Any],
...
def _display_results(result: VersionRefreshResult, verbose: bool,
                    show_unchanged: bool, check_only: bool) -> None:
