...
def update(ctx: click.Context, repository: tuple, force: bool, platform: Optional[str]):
    ...
    async def _update_cache():
        ...
        async with RepositoryManager(cache_dir, config_dir) as repo_manager:
            ...
            click.echo(
                f"Cache update completed: {success_count}/{total_count} repositories updated"
            )
    ...
    async def _clear_cache():
        ...
        async with RepositoryManager(cache_dir, config_dir) as repo_manager:
            if clear_all:
                ...
                click.echo(
                    f"Cleared cache for {
                        len(repo_names)} repositories: {removed_count} entries removed")
        ...
        async with RepositoryManager(cache_dir, config_dir) as repo_manager:
            ...
            if repositories and detailed:
                ...
                for repo_name, repo_stats in repositories.items():
                    ...
                    click.echo(
                        f"  {status_indicator} {repo_name}: {entries} entries, {
                            packages:,} packages")
                    if expired > 0:
