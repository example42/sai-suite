...
class RAGIndexer:
    ...
    def __init__(
        ...
        if not RAG_AVAILABLE:
            raise RAGError("RAG dependencies not available. Install with: pip install sai[rag]")
    ...
    async def search_similar_packages(
        ...
        query_embedding = await loop.run_in_executor(
            None, lambda: model.encode([query], convert_to_numpy=True, show_progress_bar=False)
        )
        ...
        # Search
        scores, indices = self._package_index.search(
    ...
    async def rebuild_indices(
        ...
        """
        results = {"package_index_rebuilt": False, "saidata_index_rebuilt": False}
    ...
    async def _save_package_index(
        self, index: "faiss.Index", metadata: List[Dict[str, Any]]
    ) -> None:
...
class RAGContextBuilder:
    ...
    async def build_context(
        ...
        try:
            ...
            similar_packages = await self.indexer.search_similar_packages(
                software_name, limit=max_packages
            )
            ...
            similar_saidata = await self.indexer.find_similar_saidata(
                software_name, limit=max_saidata
            )
    ...
    def _build_context_summary(
        ...
        return (
            ". ".join(summary_parts) if summary_parts else f"Building context for {software_name}"
        )
    ...
    async def _load_default_sample_saidata(self) -> List[SaiData]:
        ...
        try:
            ...
            logger.info(
                f"Loaded {len(sample_saidata)} default sample saidata files from {sample_dir}"
            )
