...
class UniversalRepositoryDownloader(BaseRepositoryDownloader):
    ...
    def __init__(self, repository_info: RepositoryInfo, config: Dict[str, Any], 
    ...
    async def download_package_list(self) -> List[RepositoryPackage]:
        ...
        if not packages_url:
            raise RepositoryError(f"No packages URL configured for repository {self.repository_info.name}")
    ...
    async def _parse_content(self, content: bytes, headers: Dict[str, str]) -> List[RepositoryPackage]:
