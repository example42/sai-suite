...
class VLLMProvider(BaseLLMProvider):
    ...
    def _validate_config(self) -> None:
        ...
        if tensor_parallel_size is not None and (not isinstance(tensor_parallel_size, int) or tensor_parallel_size < 1):
        ...
        if gpu_memory_utilization is not None and (not isinstance(gpu_memory_utilization, (int, float)) or gpu_memory_utilization <= 0 or gpu_memory_utilization > 1):
    ...
    async def generate_saidata(self, context: GenerationContext) -> LLMResponse:
        ...
        try:
            ...
            response = await self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are an expert at generating software metadata in YAML format."},
