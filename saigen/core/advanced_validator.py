...
from .validator import ValidationResult, ValidationError, ValidationSeverity, SaidataValidator
from ..utils.errors import ValidationError as ValidationException
...
class AdvancedSaidataValidator:
    ...
    async def validate_comprehensive(self, saidata: SaiData, 
                                   check_repository_accuracy: bool = True) -> QualityReport:
        ...
        metric_scores[QualityMetric.METADATA_RICHNESS] = await self._assess_metadata_richness(saidata)
        metric_scores[QualityMetric.CROSS_REFERENCE_INTEGRITY] = await self._assess_cross_reference_integrity(saidata)
        
        # Repository-dependent metrics
        repository_accuracy = {}
        if check_repository_accuracy:
            metric_scores[QualityMetric.REPOSITORY_ALIGNMENT] = await self._assess_repository_alignment(saidata)
            ...
            metric_scores[QualityMetric.REPOSITORY_ALIGNMENT] = QualityScore(
                ...
                issues=[], suggestions=["Enable repository checking for accurate assessment"]
            )
    ...
    async def _assess_completeness(self, saidata: SaiData) -> QualityScore:
        ...
        important_fields = [
            ...
            ("metadata.urls.website", saidata.metadata.urls.website if saidata.metadata.urls else None),
        ...
        completeness_score = (required_score * 0.5) + (important_score * 0.3) + (provider_score * 0.2)
        ...
        if saidata.metadata.name and saidata.metadata.display_name:
            if saidata.metadata.name.lower() != saidata.metadata.display_name.lower():
                ...
                similarity = self._calculate_string_similarity(
                    saidata.metadata.name.lower(), 
                    saidata.metadata.display_name.lower()
                )
                if similarity < 0.7:
        ...
        if saidata.providers:
            ...
            if len(all_package_names) > 1:
                ...
                if inconsistent_names:
                    package_name_issues.append(f"Package names inconsistent with metadata name: {inconsistent_names}")
                    consistency_score -= 0.1
            ...
            for provider_name, provider_config in saidata.providers.items():
                if provider_config.ports:
                    ...
                    if not provider_ports.issubset(global_ports):
                        port_issues.append(f"Provider {provider_name} has ports not in global ports")
    ...
    async def _assess_metadata_richness(self, saidata: SaiData) -> QualityScore:
        ...
        if basic_richness < 0.7:
            suggestions.append("Add more basic metadata fields (category, license, maintainer, etc.)")
        if url_richness < 0.5:
                    ...
                    for package in provider_config.packages:
                        if package.repository:
                            ...
                            if package.repository not in repository_names:
                                ...
                                issues.append(f"Package '{package.name}' references undefined repository '{package.repository}'")
                                integrity_score -= 0.1
    ...
    async def _assess_repository_alignment(self, saidata: SaiData) -> QualityScore:
        ...
        for provider_name, provider_config in saidata.providers.items():
            ...
            if provider_alignment["score"] < 0.8:
                issues.append(f"Low repository alignment for provider {provider_name}: {provider_alignment['score']:.2f}")
        ...
        for provider_name, alignment in provider_alignments.items():
            if alignment["missing_packages"]:
                suggestions.append(f"Verify package names for {provider_name}: {alignment['missing_packages']}")
    ...
    async def _assess_accuracy(self, saidata: SaiData) -> QualityScore:
                ...
                if provider_config.packages:
                    for package in provider_config.packages:
                        ...
                        if await self._package_exists_in_repository(provider_name, package.name):
                            ...
                            issues.append(f"Package '{package.name}' not found in {provider_name} repository")
        ...
        overall_accuracy = sum(accuracy_factors) / len(accuracy_factors) if accuracy_factors else 0.0
        
        if overall_accuracy < 0.8:
            suggestions.append("Review and verify package names and metadata against repository sources")
    ...
    async def _check_repository_accuracy(self, saidata: SaiData) -> Dict[str, float]:
        ...
        for provider_name, provider_config in saidata.providers.items():
            ...
            accuracy_scores[provider_name] = accurate_count / total_packages if total_packages > 0 else 0.0
    ...
    async def _check_provider_alignment(self, provider_name: str, packages: List[Any]) -> Dict[str, Any]:
    ...
    async def _get_repository_package(self, provider_name: str, package_name: str) -> Optional[RepositoryPackage]:
        ...
        try:
            ...
            if saidata.metadata.description and best_match.description:
                desc_similarity = self._calculate_string_similarity(
                    saidata.metadata.description.lower(),
                    best_match.description.lower()
                )
            ...
            if saidata.metadata.license and best_match.license:
                license_similarity = self._calculate_string_similarity(
                    saidata.metadata.license.lower(),
                    best_match.license.lower()
                )
    ...
    def _generate_recommendations(self, metric_scores: Dict[QualityMetric, QualityScore], 
        ...
        if lowest_score_metric[1].score < 0.5:
            recommendations.insert(0, f"Priority: Improve {lowest_score_metric[0].value} (score: {lowest_score_metric[1].score:.2f})")
        ...
        if saidata.providers:
            for provider_config in saidata.providers.values():
                ...
                if provider_config.directories:
                    provider_items["directories"].update(d.name for d in provider_config.directories)
                if provider_config.commands:
    ...
    def format_quality_report(self, report: QualityReport, detailed: bool = False) -> str:
        ...
        for metric, score in report.metric_scores.items():
            ...
            lines.append(f"{metric.value.replace('_', ' ').title():<25} {score_bar} {score.score:.2f}")
