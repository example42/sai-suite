...
from ..utils.checksum_validator import ChecksumValidator
from ..utils.url_templating import URLTemplateProcessor
...
class SaidataValidator:
    ...
    def __init__(self, schema_path: Optional[Path] = None):
        ...
        self.schema_path = (
            schema_path
            or Path(__file__).parent.parent.parent / "schemas" / "saidata-0.3-schema.json"
        )
                        ...
                        if isinstance(section, list):
                            for i, item in enumerate(section):
                                if isinstance(item, dict) and "checksum" in item:
                                    checksum = item["checksum"]
                                    if isinstance(checksum, str):
                                        checksum_errors = self.checksum_validator.validate_checksum(
                                            checksum
                                        )
                                        for error_msg in checksum_errors:
                                            errors.append(
                                ...
                                if repo_ref and repo_ref not in repository_names:
                                    warnings.append(
                                        ValidationError(
                                            ...
                                            suggestion=f"Define repository '{repo_ref}' or use an existing one: {
                                                ', '.join(repository_names) if repository_names else 'none defined'}",
                                        ))
    ...
    def _validate_ports(
        self, data: Dict[str, Any], source: str, warnings: List[ValidationError]
    ) -> None:
        ...
        if isinstance(providers, dict):
            for provider_name, provider_config in providers.items():
                if isinstance(provider_config, dict):
                    ...
                    check_paths_list(
                        provider_files, f"{source}.providers.{provider_name}.files", "file"
                    )

                    provider_dirs = provider_config.get("directories", [])
                    check_paths_list(
                        provider_dirs,
                        f"{source}.providers.{provider_name}.directories",
                        "directory",
                    )

    def _validate_enum_values(
        self, data: Dict[str, Any], source: str, errors: List[ValidationError]
    ) -> None:
    ...
    def attempt_recovery(
        self, data: Dict[str, Any], validation_result: ValidationResult
    ) -> RecoveryResult:
        ...
        for error in validation_result.errors:
            try:
                if self._is_url_template_error(error):
                    ...
                    recovered_data, fixed = self._fix_missing_required_field_error(
                        recovered_data, error
                    )
                    if fixed:
                        fixed_errors.append(error.message)
    ...
    def _fix_url_template_error(
        self, data: Dict[str, Any], error: ValidationError
    ...
    def _fix_checksum_format_error(
        self, data: Dict[str, Any], error: ValidationError
    ) -> tuple[Dict[str, Any], bool]:
    ...
    def _fix_missing_required_field_error(
        self, data: Dict[str, Any], error: ValidationError
    ) -> tuple[Dict[str, Any], bool]:
    ...
    def validate_with_recovery(
        self, data: Dict[str, Any], source: str = "data", max_recovery_attempts: int = 3
    ) -> tuple[ValidationResult, Optional[RecoveryResult]]:
