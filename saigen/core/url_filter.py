...
class URLValidationFilter:
    ...
    def __init__(
        ...
        user_agent: str = "SAI-URLValidator/1.0"
    ):
    ...
    def _extract_urls(self, saidata: SaiData) -> Set[str]:
        ...
        if saidata.metadata.urls:
            for field in ['website', 'documentation', 'source', 'issues', 'support', 
                         'download', 'changelog', 'license', 'sbom', 'icon']:
    ...
    def _filter_urls(self, saidata: SaiData, valid_urls: Set[str]) -> SaiData:
            ...
            for package in data_dict['packages']:
                if package.get('download_url'):
                    ...
                    if self._is_http_url(url) and url not in valid_urls:
                        logger.debug(f"Filtering out invalid download_url from package {package['name']}: {url}")
                        package['download_url'] = None
        
        # Filter sources
        if data_dict.get('sources'):
            data_dict['sources'] = [
                source for source in data_dict['sources']
        ...
        if data_dict.get('binaries'):
            data_dict['binaries'] = [
                binary for binary in data_dict['binaries']
        ...
        if data_dict.get('scripts'):
            data_dict['scripts'] = [
                script for script in data_dict['scripts']
    ...
    def _filter_provider_config(self, provider_config: Dict, valid_urls: Set[str]) -> None:
        ...
        if provider_config.get('packages'):
            for package in provider_config['packages']:
                if package.get('download_url'):
                    url = package['download_url']
                    if self._is_http_url(url) and url not in valid_urls:
                        logger.debug(f"Filtering out invalid download_url from provider package: {url}")
                        package['download_url'] = None
        ...
        if provider_config.get('repositories'):
            for repo in provider_config['repositories']:
                ...
                if repo.get('packages'):
                    for package in repo['packages']:
                        if package.get('download_url'):
                            url = package['download_url']
                            if self._is_http_url(url) and url not in valid_urls:
                                logger.debug(f"Filtering out invalid download_url from repo package: {url}")
                                package['download_url'] = None
                
                # Nested sources
                if repo.get('sources'):
                    repo['sources'] = [
                        source for source in repo['sources']
                        if not self._is_http_url(source['url']) or source['url'] in valid_urls
                    ]
                ...
                if repo.get('binaries'):
                    repo['binaries'] = [
                        binary for binary in repo['binaries']
                        if not self._is_http_url(binary['url']) or binary['url'] in valid_urls
                    ]
                ...
                if repo.get('scripts'):
                    repo['scripts'] = [
                        script for script in repo['scripts']
                        if not self._is_http_url(script['url']) or script['url'] in valid_urls
                    ]
                ...
                if pkg_source.get('packages'):
                    for package in pkg_source['packages']:
                        if package.get('download_url'):
                            url = package['download_url']
                            if self._is_http_url(url) and url not in valid_urls:
                                logger.debug(f"Filtering out invalid download_url from package source: {url}")
                                package['download_url'] = None
        
        # Filter direct sources
        if provider_config.get('sources'):
            provider_config['sources'] = [
                source for source in provider_config['sources']
                if not self._is_http_url(source['url']) or source['url'] in valid_urls
            ]
        ...
        if provider_config.get('binaries'):
            provider_config['binaries'] = [
                binary for binary in provider_config['binaries']
                if not self._is_http_url(binary['url']) or binary['url'] in valid_urls
            ]
        ...
        if provider_config.get('scripts'):
            provider_config['scripts'] = [
                script for script in provider_config['scripts']
                if not self._is_http_url(script['url']) or script['url'] in valid_urls
            ]
