...
class MCPServerClient:
    ...
    async def test_saidata(self, saidata: SaiData, providers: List[str]) -> List[SaidataTestResult]:
        ...
        if not await self.is_available():
            results.append(SaidataTestResult(
...
class SaidataTester:
    ...
    async def _test_dry_run(self, saidata: SaiData, providers: List[str]) -> List[SaidataTestResult]:
    ...
    async def _test_service_dry_run(self, provider: str, service_name: str) -> SaidataTestResult:
        ...
        """
        try:
            # Check if service exists or can be validated
            service_commands = {
                'systemd': ['systemctl', 'status', service_name],
                'init': ['service', service_name, 'status'],
                'launchd': ['launchctl', 'list', service_name]
            }
    ...
    async def _test_provider_compatibility(self, saidata: SaiData, providers: List[str]) -> List[SaidataTestResult]:
            ...
            if hasattr(entry, 'provider') and entry.provider == provider:
                ...
                if os_name in platforms or 'all' in platforms:
                    if entry.supported:
                        ...
                        severity = SaidataTestSeverity.FAIL
                        message = f"Provider {provider} marked as unsupported in compatibility matrix"
    ...
    async def _test_package_availability(self, saidata: SaiData, providers: List[str]) -> List[SaidataTestResult]:
    ...
    async def _check_package_availability(self, provider: str, package_name: str) -> SaidataTestResult:
    ...
    async def _test_command_validation(self, saidata: SaiData, providers: List[str]) -> List[SaidataTestResult]:
    ...
    async def _check_command_availability(self, command_name: str, command_path: Optional[str]) -> SaidataTestResult:
    ...
    async def _test_service_validation(self, saidata: SaiData, providers: List[str]) -> List[SaidataTestResult]:
    ...
    async def _check_service_definition(self, service_name: str, service_file_name: Optional[str]) -> SaidataTestResult:
    ...
    def format_test_report(self, test_suite: SaidataTestSuite, show_details: bool = False) -> str:
        ...
        lines.append(f"📊 Results: {test_suite.passed} passed, {test_suite.failed} failed, "
