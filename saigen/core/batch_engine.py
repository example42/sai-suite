...
from .generation_engine import GenerationEngine, GenerationEngineError
...
class BatchProgressReporter:
    ...
    def _report_progress(self, current_software: str = "") -> None:
        ...
        if self.verbose:
            ...
            print(f"\rProgress: {self.completed}/{self.total_items} ({percentage:.1f}%)", end="", flush=True)
            ...
            for line in f:
                ...
                if not line or line.startswith('#'):
                    ...
                    if line.startswith('##'):
                        ...
                        if category_filter:
                            include_section = bool(re.search(category_filter, current_category, re.IGNORECASE))
                        else:
                            include_section = True
                    continue
...
class BatchGenerationEngine:
    ...
    async def generate_batch(
        ...
        progress_reporter = BatchProgressReporter(
            total_items=len(valid_software),
        ...
        logger.info(f"Batch generation completed: {successful_count} successful, {failed_count} failed")
    ...
    def get_statistics_summary(self, result: BatchGenerationResult) -> str:
        ...
        """
        success_rate = (result.successful / result.total_requested) * 100 if result.total_requested > 0 else 0
