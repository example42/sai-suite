...
class ContextBuilderV03:
    ...
    def get_installation_method_examples(
        self, software_category: str = None
    ) -> Dict[str, List[Dict]]:
...
class EnhancedContextBuilder:
    ...
    async def _add_installation_method_context(
    ...
    async def _add_compatibility_matrix_context(
        self, context: GenerationContext
    ) -> GenerationContext:
        ...
        try:
            ...
            compatibility_template = self.compatibility_templates.get(
                compatibility_scope, self.compatibility_templates["cross_platform"]
            )
    ...
    async def _add_provider_enhancement_context(
        self, context: GenerationContext
    ) -> GenerationContext:
        ...
        try:
            provider_enhancements = {
                ...
                "prerequisites": ["build-essential", "libssl-dev", "pkg-config"],
                "build_commands": [
    ...
    def _detect_installation_methods(self, context: GenerationContext) -> List[str]:
        ...
        if context.repository_data:
            for pkg in context.repository_data:
                ...
                if any(
                    indicator in pkg_name or indicator in pkg_desc
    ...
    def _detect_software_category(self, context: GenerationContext) -> str:
        ...
        # Databases
        if any(
            term in software_name
            for term in ["mysql", "postgresql", "redis", "mongodb", "database", "db"]
        ):
            return "database"
        ...
        if any(
            term in software_name
            for term in ["python", "node", "nodejs", "java", "go", "rust", "compiler"]
        ):
            return "programming"
