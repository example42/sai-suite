...
class UpdateEngine:
        ...
        try:
            ...
            if not merge_result.success:
                return GenerationResult(
                    ...
                    validation_errors=[
                        GenValidationError(
                            field="merge", message="Failed to merge saidata", severity="error"
                        )
                    ],
                    warnings=merge_result.warnings,
                    generation_time=time.time() - start_time,
                    llm_provider_used=llm_provider.value
            ...
            await self.generation_engine.validate_saidata_file(Path("temp"))
            ...
            logger.info(
                f"Successfully updated saidata for '{
                    existing_saidata.metadata.name}' in {
                    generation_time:.2f}s")
            logger.info(
                f"Merge stats: {
                    merge_result.fields_added} added, {
                    merge_result.fields_updated} updated, {
                    merge_result.conflicts_resolved} conflicts resolved")
            ...
            return GenerationResult(
                ...
                saidata=None,
                validation_errors=[
                    GenValidationError(field="update", message=str(e), severity="error")
                ],
                warnings=[],
                generation_time=generation_time,
                llm_provider_used=llm_provider.value
    ...
    async def _merge_saidata(
        self, existing: SaiData, fresh: SaiData, strategy: MergeStrategy, interactive: bool = False
    ) -> UpdateResult:
        """Merge existing and fresh saidata.
        ...
        try:
            ...
            if strategy == MergeStrategy.PRESERVE:
                merged_dict, stats = await self._merge_preserve(
                    existing_dict, fresh_dict, interactive
                )
            elif strategy == MergeStrategy.ENHANCE:
                merged_dict, stats = await self._merge_enhance(
                    existing_dict, fresh_dict, interactive
                )
            elif strategy == MergeStrategy.REPLACE:
                merged_dict, stats = await self._merge_replace(
                    existing_dict, fresh_dict, interactive
                )
            else:
            ...
            return UpdateResult(success=False, warnings=[f"Merge failed: {e}"])

    async def _merge_preserve(
        self, existing: Dict[str, Any], fresh: Dict[str, Any], interactive: bool
        ...
        for key, value in fresh.items():
            if key not in merged:
                ...
                provider_stats = await self._merge_providers_preserve(
                    merged.get("providers", {}), fresh.get("providers", {}), interactive
                )
    ...
    async def _merge_enhance(
        ...
        if "metadata" in fresh:
            metadata_stats = await self._merge_metadata_enhance(
                merged.get("metadata", {}), fresh["metadata"], interactive
            )
        ...
        if "providers" in fresh:
            provider_stats = await self._merge_providers_enhance(
                merged.get("providers", {}), fresh["providers"], interactive
            )
            stats["fields_added"] += provider_stats["fields_added"]
            stats["fields_updated"] += provider_stats["fields_updated"]
            stats["conflicts_resolved"] += provider_stats["conflicts_resolved"]
        ...
        if interactive:
            ...
            for field_path in preserved_fields:
                self._set_nested_field(
                    merged, field_path, self._get_nested_field(existing, field_path)
                )
    ...
    async def _merge_metadata_enhance(
        self, existing_metadata: Dict[str, Any], fresh_metadata: Dict[str, Any], interactive: bool
    ) -> Dict[str, Any]:
        ...
        for key, fresh_value in fresh_metadata.items():
            if key not in existing_metadata:
                ...
                if (
                    key == "description"
                    and fresh_value
                    and len(fresh_value) > len(existing_metadata.get(key, ""))
                ):
                    # Use longer, more detailed description
                    if interactive:
                        choice = await self._prompt_field_choice(
                            f"metadata.{key}", existing_metadata[key], fresh_value
                        )
    ...
    async def _merge_providers_preserve(
        self, existing_providers: Dict[str, Any], fresh_providers: Dict[str, Any], interactive: bool
    ) -> Dict[str, Any]:
        ...
        for provider_name, fresh_config in fresh_providers.items():
            if provider_name not in existing_providers:
                ...
                provider_stats = await self._merge_provider_config(
                    existing_config, fresh_config, interactive, provider_name
                )
                stats["fields_added"] += provider_stats["fields_added"]
                stats["fields_updated"] += provider_stats["fields_updated"]
                stats["conflicts_resolved"] += provider_stats["conflicts_resolved"]
        ...
        for key, fresh_value in fresh_config.items():
            if key not in existing_config:
                ...
                merged_list = await self._merge_list_by_name(
                    existing_config[key], fresh_value, interactive, f"{provider_name}.{key}"
                )
    ...
    async def _prompt_field_choice(
        self, field_path: str, existing_value: Any, fresh_value: Any
    ) -> ConflictResolution:
    ...
    async def _prompt_preserve_fields(
        self, existing: Dict[str, Any], fresh: Dict[str, Any]
    ) -> List[str]:
